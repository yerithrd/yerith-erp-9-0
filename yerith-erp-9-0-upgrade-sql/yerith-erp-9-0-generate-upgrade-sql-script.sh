#!/bin/bash
# @file: yerith-erp-9-0-generate-upgrade-sql-script.sh
# @author: Dr.-Ing. XAVIER NOUNDOU

APP="$(basename $0)"

SQL_UPGRADE_FILE=yerith-result-last-sql-upgrade.sql
		...
		for c in ${ALL_RELEVANT_GIT_COMMITS}; do
				...
				if [ -f ${SQL_FILE} ]; then					
						...
						if [ $COUNTER -eq 0 ]; then
								...
								echo -e "-- YERITH-ERP-9.0: '.sql' file to upgrade database." >> ${SQL_UPGRADE_FILE}
