...
SRC_DIR="${YERITH_ERP_3_0_HOME_FOLDER}"

# COV_DIR is the directory where the coverage results will be stored
COV_DIR="${YERITH_ERP_3_0_HOME_FOLDER}/yerith-tests-coverage-information"
...
lcov -d "${SRC_DIR}" -c -o "${COV_DIR}/yerith-tests-coverage.info"
...
lcov -r "${COV_DIR}/yerith-tests-coverage.info" "*Qt*.framework*" "*.h" "*/tests/*" "*.moc" "*moc_*.cpp" "*/test/*" "*/build*/*" -o "${COV_DIR}/yerith-tests-coverage-filtered.info"
...
genhtml -o "${HTML_RESULTS}" \
	...
	"${COV_DIR}/yerith-tests-coverage-filtered.info"
