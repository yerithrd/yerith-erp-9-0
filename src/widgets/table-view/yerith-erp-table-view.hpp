/*
 * yerith-erp-table-view.hpp
 *
 *      Author: Pr. Prof. Dr.-Ing. Xavier Noundou
 */

#ifndef SRC_YERITH_TABLE_VIEW_HPP_
#define SRC_YERITH_TABLE_VIEW_HPP_
...
class YerithTableView : public QTableView
{
	Q_OBJECT
...
public:
    ...
    YerithTableView(QWidget *parent);

    virtual inline ~YerithTableView()
    {
    ...
    void YERITH_resizeEvent(QResizeEvent *event);
    ...
    virtual inline void lister_les_elements_du_tableau(YerithSqlTableModel &
                                                       tableModel,
                                                       const QString &
    ...
    virtual void construire_le_MAPPING_ORIGINAL_db_ID_VERS_db_row_Nr(YerithSqlTableModel &
                                                                     tableModel);
    ...
    void lister(YerithSqlTableModel &sqlTableModel,
                bool truncateString = true);

    void lister_ALL(YerithSqlTableModel &tableModel,
                    QMap<QString, QString> &stockNameToStockID_in_out);

    void lister_FIFO(YerithSqlTableModel &tableModel,
                     QMap<QString, QString> &stockNameToStockID_in_out);
    ...
    void lister_LIFO(YerithSqlTableModel &tableModel,
                     QMap<QString, QString> &stockNameToStockID_in_out);

    void lister_FEFO(YerithSqlTableModel &tableModel,
                     QMap<QString, QString> &stockNameToStockID_in_out);

    void lister_codebar_ALL(YerithSqlTableModel &tableModel,
                            QMap<QString,
                            QString> &stockReferenceToStockID_in_out);

    void lister_codebar_FIFO(YerithSqlTableModel &tableModel,
                             QMap<QString,
                             QString> &stockReferenceToStockID_in_out);

    void lister_codebar_LIFO(YerithSqlTableModel &tableModel,
                             QMap<QString,
                             QString> &stockReferenceToStockID_in_out);

    void lister_codebar_FEFO(YerithSqlTableModel &tableModel,
                             QMap<QString,
                             QString> &stockReferenceToStockID_in_out);
    ...
    inline YerithPOSQStandardItemModel *getStandardItemModel()
    {
        return _stdItemModel;
    }

    inline void setStandardItemModel(YerithPOSQStandardItemModel *
                                     stdItemModel)
    {
        _stdItemModel = stdItemModel;
    }

    static YerithERPWindows *YERITH_TABLE_VIEW_ALL_WINDOWS_POINTER;
     ...
     */
    QMap<QString, int> _map_dbID_TO_yerith_table_view_ROW_NUMBER;

    QMap<QString, QString> _MAP_lastSelected_Row__TO__DB_ID;

signals:

    void signal_lister(YerithSqlTableModel &sqlTableModel);
...
protected:
    ...
    YerithPOSQStandardItemModel *_stdItemModel;
...
#endif /* SRC_YERITH_TABLE_VIEW_HPP_ */
