/*
 * yerith-erp-fournisseurs-table-view.cpp
 *
 *      Author: Pr. Prof. Dr.-Ing. Xavier Noundou
 */


#include "yerith-erp-fournisseurs-table-view.hpp"
...
void YerithERPFournisseursTableView::
        lister_les_elements_du_tableau(YerithSqlTableModel &tableModel)
{
    ...
    QString DBTABLE_Column_TO_VIEW_employee_OR_Supplier_Ref = QObject::tr("Réf. Fournisseur");
    QString DBTABLE_Column_TO_VIEW_employee_OR_Supplier = QObject::tr("Nom Entreprise");


    if (_allWindows->_fournisseursWindow->is_Show_Employee())
    {
        DBTABLE_Column_TO_VIEW_employee_OR_Supplier_Ref = QObject::tr("Réf. Employé(e)");
        DBTABLE_Column_TO_VIEW_employee_OR_Supplier = QObject::tr("Nom Employé");
    }
...

    {
        CHARGES_FINANCIERES_WINDOW_Db_ColumnS
            .append(YerithDatabaseTableColumn::REFERENCE_FOURNISSEUR);

        CHARGES_FINANCIERES_WINDOW_Db_ColumnS
            .append(YerithDatabaseTableColumn::NOM_ENTREPRISE);
    }


    {
        CHARGES_FINANCIERES_WINDOW_Column_Custom_Viewing_STRS
            .append(DBTABLE_Column_TO_VIEW_employee_OR_Supplier_Ref);

        CHARGES_FINANCIERES_WINDOW_Column_Custom_Viewing_STRS
            .append(DBTABLE_Column_TO_VIEW_employee_OR_Supplier);
    }


    YerithUtils::createTableModelHeaders_QStringList(tableModel,
                                                     *_stdItemModel,
                                                     _tableModelHeaders,
    ...
    for (int i = 0; i < rows; ++i)
    {
        ...
        for (int k = 0; k < columns; ++k)
        {
            ...
            case QVariant::Int:
                ...
                {
                    YERITH_SAVE_ID_TO_ROW_NUMBER_FOR_YERITH_TABLE_VIEW(tmpQvString,
                                                                       qv.toInt(),
                                                                       i)
            ...
            case QVariant::Bool:
                anItem = new YerithQStandardItem(qv.toBool() ?
    ...
    curAllWindows->_fournisseursWindow->RECONDUCT_current_TableColumnUser__Viewing_Strings();
...
void YerithERPFournisseursTableView::selectionChanged(const QItemSelection &
                                                      selected,
                                                      ...
                                                      deselected)
{
    ...
    if (selectedIndexes.size() > 0)
    {
        ...
        {
            curAllWindows->
            _fournisseursWindow->getQModelIndex_dbID_from_MODEL_INDEX
