/*
 * yerith-erp-historique-du-stock-window.cpp
 *
 *      Author: Pr. Prof. Dr.-Ing. Xavier Noundou
 */

#include "yerith-erp-historique-du-stock-window.hpp"
...
//const uint YerithHistoriqueDuStockWindow::ROW_SIZE_PER_VIEWING_PAGE = 9;


YerithHistoriqueDuStockWindow::YerithHistoriqueDuStockWindow()
:YerithWindowsCommons("yerith-erp-historique-dun-stock"),
 YerithAbstractClassYerithSearchWindow(YerithDatabase::STOCKS),
 _show_ASSET(false),
 _CURRENT_PAGING_NR(1),
 _logger(new YerithLogger("YerithHistoriqueDuStockWindow"))
{
	_windowName = YerithMainWindow::get_TRANSLATED_WindowName("historique du stock");
    ...
    setYerithTableView_FROM_WINDOWS_COMMONS(tableView_historique_du_stock, false);
    ...
    setCenterAligned_FOR_YERITH_PDF_LATEX_PRINTING(2);
    setCenterAligned_FOR_YERITH_PDF_LATEX_PRINTING(3);
}
...
void YerithHistoriqueDuStockWindow::clear_CURRENT_STOCK_HISTORY()
{
    _CURRENT_PAGING_NR = 1;
...
void YerithHistoriqueDuStockWindow::listHistoriqueDuStock(const QStringList &aMouvementStockList,
                                                          ...
                                                          bool 				show_ASSET /* = false */)
{
    ...
    _previousStockID = YerithWindowsCommons::get_last_lister_selected_row_db_ID();

    if (!YerithUtils::isEqualCaseInsensitive(_previousStockID,
                                             _currentStockID))
    {
        clear_CURRENT_STOCK_HISTORY();
    }
    ...
    if (!_show_ASSET)
    {
        preambleTitle = QString("%1 - %2 -").arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
    ...
    YERITH_set_windowName_TRANSLATED
		(YerithMainWindow::get_TRANSLATED_WindowName(const_cast<char *>(preambleTitle.toStdString().c_str())),
    ...
    for (int j = 0; j < stock_history_VIEWING_PAGE_COUNT; ++j)
    {
                ...
                && s < aMouvementStockList.size(); ++i)
        {
            ...
            a_paging_viewing_STOCK_HISTORY.append(YerithHistoriqueStock::SEPARATION_EXTERNE);
        ...
        _CURRENT_PAGING_NR__TO__TABLE_VIEW_LIST.insert(k,
                                                       a_paging_viewing_STOCK_HISTORY);
...
bool YerithHistoriqueDuStockWindow::export_csv_file()
{
    bool success = false;
    ...
    QString aCurrentStockDesignation =
    			YerithUtils::YERITH_REPLACE_ALL_EMPTYSPACES_WITH_UNDERSCORE_CHARACTER(_currentStockDesignation);

#ifdef YERITH_FRANCAIS_LANGUAGE
    success = YerithUtils::export_csv_file(*this,
                                           *tableView_historique_du_stock,
                                           tableColumnsToIgnore,
                                           QString("yerith-erp-historique-stock-%1-format-csv")
    ...
    success = YerithUtils::export_csv_file(*this,
                                           *tableView_historique_du_stock,
                                           tableColumnsToIgnore,
                                           QString("yerith-erp-%1-stock-history-csv-format")
...
bool YerithHistoriqueDuStockWindow::imprimer_pdf_document()
{

    if (YerithMainWindow::LANGUE_ANGLAISE)
    {
    	_latex_template_print_pdf_content = YerithUtils::EN_template_historique_dun_stock_tex;
    }
    else
    {
    	_latex_template_print_pdf_content = YerithUtils::FR_template_historique_dun_stock_tex;
    }
    ...
    if (YerithMainWindow::LANGUE_ANGLAISE)
    {
        if (_show_ASSET)
        {
            _documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHHISTORYSTOCKASSET", "OF ASSET");
        }
        else
        {
            _documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHHISTORYSTOCKASSET", "Of Stock");
        }
    }
    else
    {
        ...
        {
            _documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHHISTORYSTOCKASSET", "DE L'IMMOBILISATION");
        }
        else
        {
            _documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHHISTORYSTOCKASSET", "Du Stock");
        }
    }
    ...
    _documentSpecificElements_FOR_PDF_LATEX_PRINTING.insert("YERITHSTOCKID",
                                                            _currentStockID);

    _documentSpecificElements_FOR_PDF_LATEX_PRINTING.insert("YERITHSTOCKHISTORYREFERENCE",
    														YerithUtils::LATEX_IN_OUT_handleForeignAccents(_currentStockReference));

    _documentSpecificElements_FOR_PDF_LATEX_PRINTING.insert("YERITHSTOCKHISTORYDESIGNATION",
    														YerithUtils::LATEX_IN_OUT_handleForeignAccents(_currentStockDesignation));

    _documentSpecificElements_FOR_PDF_LATEX_PRINTING.insert("YERITHSTOCKHISTORYSTOCKID",
    														YerithWindowsCommons::getLastListerSelectedRow__ID());

    return YerithWindowsCommons::imprimer_pdf_document();
}


void YerithHistoriqueDuStockWindow::textChangedSearchLineEditsQCompleters()
{
}


void YerithHistoriqueDuStockWindow::SLOT_line_per_pagte_MODIFIED(const QString &a_line_per_page_NR)
{
    listHistoriqueDuStock(_a_previous_mouvement_stock_list,
                          _currentStockReference,
                          _currentStockDesignation,
                          _currentStockID,
                          _show_ASSET);
}


void YerithHistoriqueDuStockWindow::appliquer_bouton_PREMIERE_PAGE()
{
    _CURRENT_PAGING_NR = 1;
...
void YerithHistoriqueDuStockWindow::appliquer_bouton_PRECEDENTE_PAGE()
{
...
void YerithHistoriqueDuStockWindow::appliquer_bouton_SUIVANTE_PAGE()
{
    _CURRENT_PAGING_NR = _CURRENT_PAGING_NR + 1;
...
void YerithHistoriqueDuStockWindow::appliquer_bouton_DERNIERE_PAGE()
{
    _CURRENT_PAGING_NR = _CURRENT_PAGING_NR__TO__TABLE_VIEW_LIST.size();
...
void YerithHistoriqueDuStockWindow::setupShortcuts()
{
...
void YerithHistoriqueDuStockWindow::setupLineEdits()
{
    lineEdit_nombre_de_lignes_par_page->setValidator(&YerithUtils::QREGEXPVALIDATOR_UINT_GREATER_THAN_ZERO);
