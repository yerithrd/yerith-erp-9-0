/*
 * YERITH_TABLEAUX_DE_BORDS_analyse_comparee_jour_semaine.cpp
 *
...
void YerithTableauxDeBordWindow::analyse_comparee_jour_semaine_VENTES_BENEFICES()
{
    ...
    do
    {
        ...
        string_benefice_semaine_query =
                        ...
                        (YerithDatabaseTableColumn::MARGE_BENEFICIAIRE,
                         YerithDatabase::STOCKS_VENDU, YerithDatabaseTableColumn::DATE_VENTE,
                         DATE_TO_DB_FORMAT_STRING(current_day_date));
...
//              qDebug() << "++ string_achats_semaine_query: " << string_achats_semaine_query;
        ...
        current_query_size =
                        YerithUtils::execQuery(qsql_query, string_benefice_semaine_query);
            ...
            ventes_info.insert(it_ventes.key(),
                               QString("\\item \\textbf{%1}: sales $\\rightarrow %2$, ")
                                .arg(YerithUtils::LATEX_IN_OUT_handleForeignAccents
                                        (YerithUtils::GET_DAYOFWEEK_FROM_QT_INT_CONSTANT(it_ventes.key())),
...
//                      QDEBUG_STRING_OUTPUT_2_N("j - VENTES 0", j);
            ...
            if (YerithMainWindow::LANGUE_ANGLAISE)
            {
                ventes_info.insert(j,
                                   QString("\\item \\textbf{%1}: sales $\\rightarrow %2$, ")
                                    .arg(YerithUtils::LATEX_IN_OUT_handleForeignAccents
                                            (YerithUtils::GET_DAYOFWEEK_FROM_QT_INT_CONSTANT(j)),
...
//      qDebug() << "++ barItems: " << barItems.values().join(YerithUtils::EMPTY_STRING);
    ...
    texDocument.replace("YERITHBARITEMS",
                        barItems.values().join(YerithUtils::EMPTY_STRING));
...
void YerithTableauxDeBordWindow::
        GEN_THREE_DIMENSION_analyse_comparee_jour_semaine_ACHATS_VENTES(QString yerithFiltre,
                                                                        ...
                                                                        const QString &comboBoxEvolutionObjetsCurrentText)
{
    ...
    article_designation_query =
                    ...
                    (YerithDatabaseTableColumn::DESIGNATION,
                     YerithDatabase::STOCKS_VENDU, YerithDatabaseTableColumn::IS_SERVICE);
    ...
    int
    current_query_size =
                    YerithUtils::execQuery(qsql_query, article_designation_query);
    ...
    for (uint k = 0; k < article_designation.size(); ++k)
    {
        ...
        do
        {
            ...
            string_achats_semaine_query =
                            QString("SELECT (%1 * %2) FROM %3 WHERE %4='%5' AND %6='%7'")
                                .arg(YerithDatabaseTableColumn::PRIX_DACHAT,
                                     ...
                                     YerithDatabase::ACHATS, YerithDatabaseTableColumn::DATE_ENTREE,
                                     DATE_TO_DB_FORMAT_STRING(current_day_date),
                                     YerithDatabaseTableColumn::DESIGNATION,
                                     current_designation);
            ...
            current_query_size =
                            YerithUtils::execQuery(qsql_query, string_achats_semaine_query);
        ...
        do
        {
            ...
            string_chiffre_daffaire_semaine_query =
                            ...
                            (YerithDatabaseTableColumn::MONTANT_TOTAL_VENTE,
                             ...
                             YerithDatabaseTableColumn::DATE_VENTE,
                             DATE_TO_DB_FORMAT_STRING(current_day_date),
                             YerithDatabaseTableColumn::DESIGNATION, current_designation);
    ...
    /*
    	...
    	_reportTexFileEndString.append(YerithUtils::LATEX_IN_OUT_handleForeignAccents(
    ...
    QString YERITHDEFINETOPCOLUMNCOLORS;

    for (int i = 1; i <= 7; ++i)
    {
        YERITHDEFINETOPCOLUMNCOLORS.append(QString("\\def\\%1col{%2}\n"
                                                   "\\def\\%3topcol{%4}\n").
                                           arg
    ...
    //QDEBUG_STRING_OUTPUT_2("YERITHDEFINETOPCOLUMNCOLORS", YERITHDEFINETOPCOLUMNCOLORS);
    ...
    while (it_ventes.hasNext())
    {
        ...
        for (uint j = 0; j < days_of_week.size(); ++j)
        {
            ...
            current_week_day =
                            YerithUtils::GET_DAYOFWEEK_FROM_QT_INT_CONSTANT(k);
    ...
    QString YERITHDEFINEBARSUBTOPBARITEMS;
    ...
    for (uint i = 0; i < products_designations.size(); ++i)
    {
        ...
        for (uint k = 1; k <= 7; ++k)
        {
            YERITHDEFINEBARSUBTOPBARITEMS.append(QString("%1").
                                                 arg(day_TO_QStringBaritem.
                                                     value(k)));
        }

        YERITHDEFINEBARSUBTOPBARITEMS.append("%---\n");
    }

    //QDEBUG_STRING_OUTPUT_2("YERITHDEFINEBARSUBTOPBARITEMS", YERITHDEFINEBARSUBTOPBARITEMS);

    QString YERITHSUBGROUPS;
    ...
    for (uint k = 0; k < products_designations.size(); ++k)
    {
        ...
        {
            YERITHSUBGROUPS.append(QString(" \\centering %1 \& ").arg
                                   (YerithUtils::
                                    LATEX_IN_OUT_handleForeignAccents
    ...
    if (YerithMainWindow::LANGUE_ANGLAISE)
    {
    	texDocument.append(YerithUtils::EN_template_yerith_erp_3D_BARDIAG_TEMPLATE_tex);
    }
    else
    {
    	texDocument.append(YerithUtils::FR_template_yerith_erp_3D_BARDIAG_TEMPLATE_tex);
    }


    texDocument.replace("YERITHSUBGROUPS", YERITHSUBGROUPS);

    texDocument.replace("YERITHDEFINETOPCOLUMNCOLORS", YERITHDEFINETOPCOLUMNCOLORS);

    texDocument.replace("YERITHDEFINEBARSUBTOPBARITEMS", YERITHDEFINEBARSUBTOPBARITEMS);
    texDocument.replace("YERITHBASELEGEND",
                        QObject::tr("\\diagLegenditem{Ratio du chiffre d'affaire jour--semaine.}{\\onecol}"));

    texDocument.replace("YERITHTOPLEGEND",
                        QObject::tr("\\diagLegenditem{Ratio des achats jour--semaine.}{\\onetopcol}"));

    texDocument.replace("YERITHX", QObject::tr("Jours de semaines."));

    texDocument.replace("YERITHZ",
                        QObject::tr("Ratio du chiffre d'affaire jour--semaine."));

    QString fileName1(YerithERPConfig::temporaryFilesDir +
    				  "/YERITH-ERP-3D-BARDIAG-TEMPLATE.tex");
    ...
    if (YerithMainWindow::LANGUE_ANGLAISE)
    {
        texDocument2.append(YerithUtils::EN_template_THREE_D_bardiag_tex);
    }
    else
    {
        texDocument2.append(YerithUtils::FR_template_THREE_D_bardiag_tex);
    }
    ...
    texDocument2.replace("YERITHSUBJECT",
                         QObject::tr
    ...
    texDocument2.replace("YERITH-ERP-3D-BARDIAG-TEMPLATE.tex", fileName1);
...
void YerithTableauxDeBordWindow::analyse_comparee_jour_semaine_ACHATS_VENTES()
{
    ...
    if (_GENERATE_THREE_D_BARD_DIAGRAM)
    {
        ...
        (yerithFiltre, textFromLineEditEvolutionSujets,
         comboBoxEvolutionObjetsCurrentText);
    ...
    do
    {
        ...
        string_achats_semaine_query =
                        ...
                        (YerithDatabaseTableColumn::PRIX_DACHAT,
                         YerithDatabaseTableColumn::QUANTITE_TOTALE, YerithDatabase::ACHATS,
    ...
    if (YerithMainWindow::LANGUE_ANGLAISE)
    {
        longDateDebut =
            QString("'%1'")
                .arg(YerithUtils::LATEX_IN_OUT_handleForeignAccents
                        (YerithUtils::englishLocale.toString(dateEdit_chiffre_daffaire_jour_semaine_debut->date())));

        longDateFin =
            QString("'%1'")
                .arg(YerithUtils::LATEX_IN_OUT_handleForeignAccents
                        (YerithUtils::englishLocale.toString(dateEdit_chiffre_daffaire_jour_semaine_fin->date())));
    }
    else
    {
        longDateDebut =
            QString("'%1'")
                .arg(YerithUtils::LATEX_IN_OUT_handleForeignAccents
                        (YerithUtils::frenchLocale.toString(dateEdit_chiffre_daffaire_jour_semaine_debut->date())));

        longDateFin =
            QString("'%1'")
                .arg(YerithUtils::LATEX_IN_OUT_handleForeignAccents
                        (YerithUtils::frenchLocale.toString(dateEdit_chiffre_daffaire_jour_semaine_fin->date())));
    }
