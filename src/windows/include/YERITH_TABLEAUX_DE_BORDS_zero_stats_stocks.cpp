/*
 * YERITH_TABLEAUX_DE_BORDS_zero_stats_stocks.cpp
 *
...
void YerithTableauxDeBordWindow::ZERO_stats_stocks(QString fileName,
                                                   QString fieldId)
{
    ...
    QString strQuery(QString("SELECT %1, prix_dachat, quantite_totale "
                     ...
                     arg(fieldId, YerithDatabase::STOCKS, fieldId, fieldId,
                         YerithDatabase::STOCKS_VENDU, fieldId, fieldId,
...
//    QDEBUG_STRING_OUTPUT_2_N("querySize", querySize);
    ...
    qSort(allItems.begin(), allItems.end(),
          Yerith_MAP_COMPLEX_Item::lessThan_second_value);
    ...
    for (int j = allItems.size() - 1, k = 0; j > -1; --j, ++k)
    {
        ...
        value.append(YerithUtils::LATEX_IN_OUT_handleForeignAccents
