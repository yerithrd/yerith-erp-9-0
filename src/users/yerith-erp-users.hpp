...
#ifndef SRC_YERITH_ERP_USERS_HPP_
#define SRC_YERITH_ERP_USERS_HPP_


#include "src/users/yerith-erp-user-settings.hpp"
...
class YerithERPUserSettings;
class YerithPOSUserAdministrateur;
class YerithPOSUserManager;
class YerithERPVendeur;
class YerithPOSUserGestionaireDesStocks;
class YerithPOSUserMagasinier;
class YerithPOSUserCaissier;

class YerithPOSUser : public QObject
{
	Q_OBJECT
...
public:
	...
	YerithPOSUser(YerithERPWindows *allWindows);


    virtual ~YerithPOSUser();


    enum YerithUtils::USER_ROLE role()
    {
        return _role;
    }
    ...
    inline void setRole(enum YerithUtils::USER_ROLE aRole)
    {
        _role = aRole;
    }
    ...
    {
        return YerithUtils::LATEX_IN_OUT_handleForeignAccents(_nom_complet);
    }
    ...
    {
        return YerithUtils::LATEX_IN_OUT_handleForeignAccents(_titre);
    }
    ...
    {
        return _role == YerithUtils::ROLE_INDEFINI;
    }

    virtual inline bool isAdmin()
    {
        return _role == YerithUtils::ROLE_ADMINISTRATEUR;
    }

    virtual inline bool isManager()
    {
        return _role == YerithUtils::ROLE_MANAGER;
    }

    virtual inline bool isVendeur()
    {
        return _role == YerithUtils::ROLE_VENDEUR;
    }

    virtual inline bool isMagasinier()
    {
        return _role == YerithUtils::ROLE_MAGASINIER;
    }

    virtual inline bool isGestionaireDesStocks()
    {
        return _role == YerithUtils::ROLE_GESTIONNAIREDESTOCK;
    }

    virtual inline bool isCaissier()
    {
        return _role == YerithUtils::ROLE_CAISSIER;
    }
    ...
    inline QString GET_WINDOWS_PRINT_TABLE_ROW_COUNT_KEY_TO_STORE(const QString &a_window_object_name)
    {
        return (0 !=
               ...
               (a_window_object_name) : YerithUtils::EMPTY_STRING;
    }
...
protected:
    ...
    enum YerithUtils::USER_ROLE _role;
    ...
    QSemaphore *_write_read_user_setting_file_YERITH_SEMAPHORE;

    YerithERPUserSettings *_user_personal_settings;
...
#endif /* SRC_YERITH_ERP_USERS_HPP_ */
