/**
 * yerith-erp-admin-lister-window.cpp
 *
 *      Author: Pr. Prof. Dr.-Ing. Xavier Noundou
 */

#include "yerith-erp-admin-lister-window.hpp"

#include "src/yerith-erp-windows.hpp"

#ifdef YERITH_SERVER

	#include "src/dbus/YERITH_RUNTIME_VERIFIER.hpp"

#endif


#include "src/admin/admin-actions-subjects.hpp"


#include "src/utils/yerith-erp-utils.hpp"

#include "src/admin/yerith-erp-admin-search-form.hpp"


#include "src/utils/yerith-erp-CANCEL-Modification-TIMING-Object.hpp"


#include <QtCore/QDebug>

#include <QtWidgets/QDesktopWidget>

#include <QtGui/QStandardItemModel>

#include <QtSql/QSqlQuery>

#include <QtSql/QSqlError>



YerithAdminListerWindow::YerithAdminListerWindow()
:YerithPOSAdminWindowsCommons(QObject::tr("administration ~ lister")),
 _alertCurrentlyFiltered(false),
 _productDepartmentCurrentlyFiltered(false),
 _categoryCurrentlyFiltered(false),
 _LIGNE_BUDGETAIRE_CurrentlyFiltered(false),
 _bankAccountCurrentlyFiltered(false),
 _userCurrentlyFiltered(false),
 _siteCurrentlyFiltered(false),
 _charges_financieres_CurrentlyFiltered(false),
 _discountCurrentlyFiltered(false),
 _pushButton_admin_rechercher_font(0),
 _logger(new YerithLogger("YerithAdminListerWindow")),
 _adminSearchForm(0),
 _curSearchSqlTableModel(0),
 _lastItemSelectedForModification(0)
{
	//SETS A PREFIX FOR printed pdf document of this page
	_output_print_pdf_latexFileNamePrefix = QObject::tr("ADMINISTRATION_LISTER_PDF");

    setupUi(this);

    mySetupUi(this);

    setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_alerte);


    _list_actions_to_enable_on_positive_tableview_ROW_COUNT
		<< action_CREER_A_PARTIR_DE_CECI
		<< actionAUGMENTER_LA_POLICE_DU_TABLEAU
		<< actiondiminuer_la_police_du_tableau
        << actionExporter_au_format_csv
        << action_parametrer_les_impressions
        << actionAfficherPDF;


    setup_select_configure_dbcolumn(YerithDatabase::ALERTES);


    lineEdit_nombre_de_resultats->setYerithEnabled(false);


    QMESSAGE_BOX_STYLE_SHEET =
                    QString("QMessageBox {background-color: rgb(%1);}"
                            "QMessageBox QLabel {color: rgb(%2);}").arg
                    (COLOUR_RGB_STRING_YERITH_DARK_GREEN_47_67_67,
                     COLOUR_RGB_STRING_YERITH_GREEN_2_160_170);

    _pushButton_admin_rechercher_font =
                    new QFont(pushButton_admin_rechercher->font());

    _adminSearchForm = new YerithAdminSearchForm(_allWindows, this);


    YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, false);

    YERITH_ERP_ADMIN_WRAPPER_QACTION_SET_ENABLED(actionQui_suis_je, true);

    YERITH_ERP_ADMIN_WRAPPER_QACTION_SET_ENABLED(actionRetournerMenuPrincipal, false);

    YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(actionExporter_au_format_csv, false);


    MACRO_TO_DISABLE_PAGE_FIRST_NEXT_PREVIOUS_LAST_PUSH_BUTTONS


    tableView_lister_utilisateur->setSqlTableName(&YerithDatabase::USERS);

    tableView_lister_localisation
		->setSqlTableName(&YerithDatabase::LOCALISATIONS);

    tableView_lister_departements_produits
		->setSqlTableName(&YerithDatabase::DEPARTEMENTS_PRODUITS);

    tableView_lister_categorie->setSqlTableName(&YerithDatabase::CATEGORIES);

    tableView_lister_LIGNE_BUDGETAIRE->setSqlTableName(&YerithDatabase::LIGNES_BUDGETAIRES);

    tableView_lister_compte_bancaire->setSqlTableName(&YerithDatabase::COMPTES_BANCAIRES);

    tableView_lister_remise->setSqlTableName(&YerithDatabase::REMISES);

    tableView_lister_alerte->setSqlTableName(&YerithDatabase::ALERTES);


    _windowName = QString("%1 - %2")
                			.arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
                				 QObject::tr("administration ~ lister"));


    pushButton_menu->enable(this, SLOT(Main_MENU()));

    pushButton_creer->enable(this, SLOT(creer()));

    pushButton_modifier->enable(this, SLOT(modifier()));

    pushButton_afficher->enable(this, SLOT(afficher_au_detail()));

    pushButton_supprimer->enable(this, SLOT(supprimer()));

    pushButton_admin_reinitialiser->enable(this, SLOT(reinitialiser()));

    pushButton_admin_rechercher->enable(this, SLOT(rechercher()));



    connect(action_CREER_A_PARTIR_DE_CECI,
    		SIGNAL(triggered()),
			this,
			SLOT(SLOT_CREER_A_PARTIR_DE_CECI()));


    connect(actionAfficher, SIGNAL(triggered()), this,
            SLOT(afficher_au_detail()));


    connect(actionReinitialiserChampsDBVisible,
    		SIGNAL(triggered()),
			this,
			SLOT(slot_reinitialiser_colones_db_visibles()));

    connect(actionChampsDBVisible,
    		SIGNAL(triggered()),
			this,
			SLOT(selectionner_champs_db_visibles()));


    connect(actionAUGMENTER_LA_POLICE_DU_TABLEAU,
    		SIGNAL(triggered()),
    		this,
            SLOT(incrementFontSize__OF_TABLE()));

    connect(actiondiminuer_la_police_du_tableau,
    		SIGNAL(triggered()),
    		this,
            SLOT(decrementFontSize__OF_TABLE()));


    connect(actionExporter_au_format_csv,
            SIGNAL(triggered()),
            this,
            SLOT(export_csv_file()));


    connect(action_parametrer_les_impressions,
    		SIGNAL(triggered()),
			this,
            SLOT(setup_print()));

    connect(actionAfficherPDF,
    		SIGNAL(triggered()),
			this,
            SLOT(print_PDF_PREVIOUSLY_SETUP()));


    connect(actionCreer, SIGNAL(triggered()), this, SLOT(creer()));

    connect(actionMenu, SIGNAL(triggered()), this, SLOT(Main_MENU()));

    connect(actionModifier, SIGNAL(triggered()), this, SLOT(modifier()));

    connect(actionSupprimer, SIGNAL(triggered()), this, SLOT(supprimer()));

    connect(actionQui_suis_je, SIGNAL(triggered()), this,
            SLOT(qui_suis_je()));

    connect(tabWidget_lister,
    		SIGNAL(currentChanged(int)),
			this,
            SLOT(handleCurrentChanged(int)));



    connect(tableView_lister_utilisateur, SIGNAL(clicked(QModelIndex)), this,
            SLOT(handleItemModification(QModelIndex)));

    connect(tableView_lister_localisation, SIGNAL(clicked(QModelIndex)),
            this, SLOT(handleItemModification(QModelIndex)));

    connect(tableView_lister_departements_produits,
            SIGNAL(clicked(QModelIndex)),
			this,
            SLOT(handleItemModification(QModelIndex)));

    connect(tableView_lister_categorie,
    		SIGNAL(clicked(QModelIndex)),
    		this,
            SLOT(handleItemModification(QModelIndex)));

    connect(tableView_lister_charges_financieres,
    		SIGNAL(clicked(QModelIndex)),
			this,
            SLOT(handleItemModification(QModelIndex)));

    connect(tableView_lister_LIGNE_BUDGETAIRE,
    		SIGNAL(clicked(QModelIndex)),
			this,
            SLOT(handleItemModification(QModelIndex)));

    connect(tableView_lister_compte_bancaire,
    		SIGNAL(clicked(QModelIndex)),
            this,
			SLOT(handleItemModification(QModelIndex)));

    connect(tableView_lister_alerte,
    		SIGNAL(clicked(QModelIndex)),
			this,
            SLOT(handleItemModification(QModelIndex)));



    connect(tableView_lister_utilisateur,
            SIGNAL(doubleClicked(const QModelIndex &)),
			this,
            SLOT(afficher_detail_utilisateur()));

    connect(tableView_lister_localisation,
            SIGNAL(doubleClicked(const QModelIndex &)),
			this,
            SLOT(afficher_detail_localisation()));

    connect(tableView_lister_departements_produits,
            SIGNAL(doubleClicked(const QModelIndex &)),
			this,
            SLOT(afficher_detail_departements_de_produits()));

    connect(tableView_lister_categorie,
            SIGNAL(doubleClicked(const QModelIndex &)),
			this,
            SLOT(afficher_detail_categorie()));

    connect(tableView_lister_charges_financieres,
            SIGNAL(doubleClicked(const QModelIndex &)),
			this,
            SLOT(afficher_detail_CHARGE_FINANCIERE()));

    connect(tableView_lister_LIGNE_BUDGETAIRE,
            SIGNAL(doubleClicked(const QModelIndex &)),
			this,
            SLOT(afficher_detail_LIGNE_BUDGETAIRE()));

    connect(tableView_lister_compte_bancaire,
            SIGNAL(doubleClicked(const QModelIndex &)),
			this,
            SLOT(afficher_detail_compte_bancaire()));

    connect(tableView_lister_remise,
            SIGNAL(doubleClicked(const QModelIndex &)),
			this,
            SLOT(afficher_detail_remise()));

    connect(tableView_lister_charges_financieres,
            SIGNAL(doubleClicked(const QModelIndex &)),
			this,
            SLOT(afficher_detail_CHARGE_FINANCIERE()));

    connect(tableView_lister_alerte,
            SIGNAL(doubleClicked(const QModelIndex &)),
			this,
            SLOT(afficher_detail_alerte()));


    /** Menu actions */
    connect(actionChanger_utilisateur, SIGNAL(triggered()), this,
            SLOT(changer_utilisateur()));

    connect(actionDeconnecter_utilisateur, SIGNAL(triggered()), this,
            SLOT(deconnecter_utilisateur()));

    connect(actionAppeler_aide, SIGNAL(triggered()), this, SLOT(help()));

    connect(actionFermeture, SIGNAL(triggered()), this, SLOT(fermeture()));

    connect(actionA_propos, SIGNAL(triggered()), this, SLOT(apropos()));

    connect(actionRetournerMenuPrincipal, SIGNAL(triggered()), this,
            SLOT(retour_menu_principal()));


    connect(action_rechercher_tableau_liste,
            SIGNAL(triggered()),
            this,
            SLOT(rechercher()));


    setCenterAligned_FOR_YERITH_PDF_LATEX_PRINTING
        (YerithDatabaseTableColumn::YRI_Not_PHYSICAL_ASSET_STOCK_DEPARTMENT);


    set_shortcuts();
}


YerithAdminListerWindow::~YerithAdminListerWindow()
{
    delete _pushButton_admin_rechercher_font;
    delete _logger;
    delete _adminSearchForm;
}


void YerithAdminListerWindow::definirPasDeRole()
{
    _logger->log("definirPasDeRole");
    YERITH_ERP_ADMIN_WRAPPER_QACTION_SET_ENABLED(actionDeconnecter_utilisateur, false);
    YERITH_ERP_ADMIN_WRAPPER_QACTION_SET_ENABLED(actionRetournerMenuPrincipal, false);

    YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(actionExporter_au_format_csv, false);

    MACRO_TO_DISABLE_PAGE_FIRST_NEXT_PREVIOUS_LAST_PUSH_BUTTONS
}


void YerithAdminListerWindow::definirAdministrateur()
{
    _logger->log("definirAdministrateur");
    YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, false);

    YERITH_ERP_ADMIN_WRAPPER_QACTION_SET_ENABLED(actionDeconnecter_utilisateur, true);
    YERITH_ERP_ADMIN_WRAPPER_QACTION_SET_ENABLED(actionRetournerMenuPrincipal, false);

    YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(actionExporter_au_format_csv, true);

    MACRO_TO_ENABLE_PAGE_FIRST_NEXT_PREVIOUS_LAST_PUSH_BUTTONS(this,
    														   _curSearchSqlTableModel)
}


void YerithAdminListerWindow::definirManager()
{
    _logger->log("definirManager");

    YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, true);

    YERITH_ERP_ADMIN_WRAPPER_QACTION_SET_ENABLED(actionDeconnecter_utilisateur, true);
    YERITH_ERP_ADMIN_WRAPPER_QACTION_SET_ENABLED(actionRetournerMenuPrincipal, true);


    YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(actionExporter_au_format_csv, true);


    MACRO_TO_ENABLE_PAGE_FIRST_NEXT_PREVIOUS_LAST_PUSH_BUTTONS(this,
    														   _curSearchSqlTableModel)
}


void YerithAdminListerWindow::self_reset_view(int currentIndex)
{
    rendreInvisible();
    rendreVisible(currentIndex);
}


void YerithAdminListerWindow::Enable_OR_disable_actions_ON_Empty_ROW_COUNT(YerithTableView *aQtable_Yerith_VIEW)
{
    if (0 != aQtable_Yerith_VIEW        &&
        aQtable_Yerith_VIEW->rowCount() > 0)
    {
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(actionAUGMENTER_LA_POLICE_DU_TABLEAU, true);
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(actiondiminuer_la_police_du_tableau, true);
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_parametrer_les_impressions, true);
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(actionAfficherPDF, true);
    }
    else
    {
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(actionAUGMENTER_LA_POLICE_DU_TABLEAU, false);
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(actiondiminuer_la_police_du_tableau, false);
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_parametrer_les_impressions, false);
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(actionAfficherPDF, false);
    }
}


void YerithAdminListerWindow::rendreVisible(unsigned selectedSujetAction)
{
    HANDLE_rendering_IF_ALLOWED_PREVIOUSLY();

    retranslateUi(this);

    _windowName = _windowName_WITH_NO_MAINTENANCE;

    switch (selectedSujetAction)
    {
    case SUJET_ACTION_COMPTE_UTILISATEUR:
        if (false == isUserCurrentlyFiltered())
        {
            setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_utilisateur, false);

            lister_utilisateur();

            Enable_OR_disable_actions_ON_Empty_ROW_COUNT(tableView_lister_utilisateur);
        }
        break;

    case SUJET_ACTION_LOCALISATION:
        if (false == isSiteCurrentlyFiltered())
        {
            setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_localisation);

            lister_localisation();

            Enable_OR_disable_actions_ON_Empty_ROW_COUNT(tableView_lister_localisation);
        }
        break;

    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:
        if (false == isProductDepartmentCurrentlyFiltered())
        {
            setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_departements_produits);

            lister_departements_de_produits();

            Enable_OR_disable_actions_ON_Empty_ROW_COUNT(tableView_lister_departements_produits);
        }
        break;

    case SUJET_ACTION_CATEGORIE:
        if (false == isCategoryCurrentlyFiltered())
        {
            setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_categorie);

            lister_categorie();

            Enable_OR_disable_actions_ON_Empty_ROW_COUNT(tableView_lister_categorie);
        }
        break;

    case SUJET_ACTION_ligne_budgetaire:
        if (false == is_LIGNE_BUDGETAIRE_CurrentlyFiltered())
        {
            setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_LIGNE_BUDGETAIRE);

            lister_LIGNE_BUDGETAIRE();

            Enable_OR_disable_actions_ON_Empty_ROW_COUNT(tableView_lister_LIGNE_BUDGETAIRE);

            static bool first_Execution_time = true;

            if (first_Execution_time)
            {
                YerithQMessageBox::information(this,
                                               QObject::tr("admin-lister LIGNE BUDGETAIRE"),
                                               QObject::tr("Créer 1 LIGNE BUDGETAIRE nécessite "
                                                           "avoir créée 1 compte bancaire "
                                                           "ici à la section Administration !"));

                first_Execution_time = false;
            }
        }
        break;

    case SUJET_ACTION_COMPTE_BANCAIRE:
        if (false == isBankAccountCurrentlyFiltered())
        {
            setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_compte_bancaire);

            lister_compte_bancaire();

            Enable_OR_disable_actions_ON_Empty_ROW_COUNT(tableView_lister_compte_bancaire);
        }
        break;

    case SUJET_ACTION_ALERTE:
        if (false == isAlertCurrentlyFiltered())
        {
            setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_alerte);

            lister_alerte();

            Enable_OR_disable_actions_ON_Empty_ROW_COUNT(tableView_lister_alerte);
        }
        break;

    case SUJET_ACTION_REMISE:
        if (false == isDiscountCurrentlyFiltered())
        {
            setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_remise);

            lister_remise();

            Enable_OR_disable_actions_ON_Empty_ROW_COUNT(tableView_lister_remise);
        }
        break;

    case SUJET_ACTION_CHARGE_FINANCIERE:
        if (false == isCharge_financieres_CurrentlyFiltered())
        {
            setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_charges_financieres);

            LISTER_CHARGES_FINANCIERES();

            Enable_OR_disable_actions_ON_Empty_ROW_COUNT(tableView_lister_charges_financieres);
        }
        break;

    default:
        break;
    }

    tabWidget_lister->setCurrentIndex(selectedSujetAction);

    setVisible(true);
}


void YerithAdminListerWindow::rechercher()
{
    _adminSearchForm->rendreVisible(tabWidget_lister->currentIndex());
}


void YerithAdminListerWindow::reinitialiser()
{
    _logger->log("reinitialiser");

    _adminSearchForm->reinitialiser();
    _adminSearchForm->rendreInvisible();

    if (_curSearchSqlTableModel)
    {
        _curSearchSqlTableModel->resetFilter("src/admin/lister/yerith-erp-admin-lister-window.cpp", 459);
        _curSearchSqlTableModel->easySelect("src/admin/lister/yerith-erp-admin-lister-window.cpp", 460);
    }

    switch (tabWidget_lister->currentIndex())
    {
    case SUJET_ACTION_COMPTE_UTILISATEUR:
    	setUserCurrentlyFiltered(false);
    	lister_utilisateur();
        break;

    case SUJET_ACTION_LOCALISATION:
    	setSiteCurrentlyFiltered(false);
    	lister_localisation();
        break;

    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:
    	setProductDepartmentCurrentlyFiltered(false);
    	lister_departements_de_produits();
        break;

    case SUJET_ACTION_CATEGORIE:
    	setCategoryCurrentlyFiltered(false);
    	lister_categorie();
        break;

    case SUJET_ACTION_ligne_budgetaire:
    	setLIGNE_BUDGETAIRE_CurrentlyFiltered(false);
    	lister_LIGNE_BUDGETAIRE();
        break;

    case SUJET_ACTION_COMPTE_BANCAIRE:
    	setBankAccountCurrentlyFiltered(false);
    	lister_compte_bancaire();
        break;

    case SUJET_ACTION_ALERTE:
    	setAlertCurrentlyFiltered(false);
    	lister_alerte();
        break;

    case SUJET_ACTION_REMISE:
    	setDiscountCurrentlyFiltered(false);
    	lister_remise();
        break;

    case SUJET_ACTION_CHARGE_FINANCIERE:
    	setCharge_financieres_CurrentlyFiltered(false);
    	LISTER_CHARGES_FINANCIERES();
        break;

    default:
        break;
    }

    setCurSearchSqlTableModel(0);

    set_admin_rechercher_font();
}


void YerithAdminListerWindow::set_admin_rechercher_font()
{
    switch (tabWidget_lister->currentIndex())
    {
    case SUJET_ACTION_COMPTE_UTILISATEUR:
    	MACRO_SET_ADMIN_RECHERCHER_FONT(_userCurrentlyFiltered)
		setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_utilisateur"));

		break;

    case SUJET_ACTION_LOCALISATION:
    	_curSearchSqlTableModel = &_allWindows->getSqlTableModel_localisations();
        MACRO_SET_ADMIN_RECHERCHER_FONT(_siteCurrentlyFiltered)
		setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_localisation"));

        break;

    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:
    	_curSearchSqlTableModel = &_allWindows->getSqlTableModel_departements_produits();
        MACRO_SET_ADMIN_RECHERCHER_FONT(_productDepartmentCurrentlyFiltered)
		setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_departements_de_produits"));

		break;

    case SUJET_ACTION_CATEGORIE:
    	_curSearchSqlTableModel = &_allWindows->getSqlTableModel_categories();
    	MACRO_SET_ADMIN_RECHERCHER_FONT(_categoryCurrentlyFiltered)
		setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_categorie"));

        break;

    case SUJET_ACTION_ligne_budgetaire:
    	_curSearchSqlTableModel = &_allWindows->getSqlTableModel_lignes_budgetaires();
        MACRO_SET_ADMIN_RECHERCHER_FONT(_LIGNE_BUDGETAIRE_CurrentlyFiltered)
		setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_LIGNE_BUDGETAIRE"));

        break;

    case SUJET_ACTION_COMPTE_BANCAIRE:
    	_curSearchSqlTableModel = &_allWindows->getSqlTableModel_comptes_bancaires();
        MACRO_SET_ADMIN_RECHERCHER_FONT(_bankAccountCurrentlyFiltered)
		setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_compte_bancaire"));

        break;

    case SUJET_ACTION_REMISE:
    	_curSearchSqlTableModel = &_allWindows->getSqlTableModel_remises();
        MACRO_SET_ADMIN_RECHERCHER_FONT(_discountCurrentlyFiltered)
		setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_remise"));

        break;

    case SUJET_ACTION_ALERTE:
    	_curSearchSqlTableModel = &_allWindows->getSqlTableModel_alertes();
        MACRO_SET_ADMIN_RECHERCHER_FONT(_alertCurrentlyFiltered)
		setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_alerte"));

        break;

    case SUJET_ACTION_CHARGE_FINANCIERE:
    	_curSearchSqlTableModel = &_allWindows->getSqlTableModel_charges_financieres();
        MACRO_SET_ADMIN_RECHERCHER_FONT(_charges_financieres_CurrentlyFiltered)
		setWindowTitle(_LISTER_tab_TO_tabTitle.value("LISTER_CHARGES_FINANCIERES"));

        break;

    default:
    	_curSearchSqlTableModel = 0;
    	setWindowTitle(_windowName_WITH_NO_MAINTENANCE);
        break;
    }

    pushButton_admin_rechercher->setFont(*_pushButton_admin_rechercher_font);
}


int YerithAdminListerWindow::
		LIST_SHOW_TABLE_VIEW_WITH_PAGINATION
			(YerithTableViewWITHpagination  &a_table_view_to_list_show,
			 YerithSqlTableModel 			&aSqlTableModel)
{
	//I UPDATE A CURRENT DATABASE COLUMN SECTION DIALOG
	_selectExportDBQDialog =
			GET_ERP_GENERIC_SELECTDBFIELD_DIALOG(aSqlTableModel.sqlTableName());

    a_table_view_to_list_show
		.queryYerithTableViewCurrentPageContentRow(aSqlTableModel);

    tableView_show_or_hide_columns(a_table_view_to_list_show);

    int rowCount = a_table_view_to_list_show.rowCount();

    lineEdit_nombre_de_resultats->setText(GET_NUM_STRING(rowCount));

    return rowCount;
}


YerithTableViewWITHpagination *YerithAdminListerWindow::GET_CURRENT_TABLEVIEW()
{
	YerithTableViewWITHpagination *result = 0;

	switch (tabWidget_lister->currentIndex())
	    {
	    case SUJET_ACTION_COMPTE_UTILISATEUR:
	    	result = tableView_lister_utilisateur;
	        break;

	    case SUJET_ACTION_LOCALISATION:
	    	result = tableView_lister_localisation;
	        break;

	    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:
	    	result = tableView_lister_departements_produits;
	        break;

	    case SUJET_ACTION_CATEGORIE:
	    	result = tableView_lister_categorie;
	        break;

	    case SUJET_ACTION_ligne_budgetaire:
	    	result = tableView_lister_LIGNE_BUDGETAIRE;
	        break;

	    case SUJET_ACTION_COMPTE_BANCAIRE:
	    	result = tableView_lister_compte_bancaire;
	        break;

	    case SUJET_ACTION_ALERTE:
	    	result = tableView_lister_alerte;
	        break;

	    case SUJET_ACTION_REMISE:
	    	result = tableView_lister_remise;
	        break;

	    case SUJET_ACTION_CHARGE_FINANCIERE:
	    	result = tableView_lister_charges_financieres;
	        break;

	    default:
	        break;
	    }

	return result;
}


void YerithAdminListerWindow::RESET_VIEW_CHARGES_FINANCIERES()
{
    _visibleDBColumnNameStrList.clear();

    _visibleDBColumnNameStrList
		<< YerithDatabaseTableColumn::DESIGNATION
		<< YerithDatabaseTableColumn::NOM_ENTREPRISE_FOURNISSEUR
		<< YerithDatabaseTableColumn::REFERENCE_RECU_DACHAT
		<< YerithDatabaseTableColumn::PRIX_DACHAT
		<< YerithDatabaseTableColumn::PRIX_UNITAIRE
		<< YerithDatabaseTableColumn::DATE_DE_RECEPTION
		<< YerithDatabaseTableColumn::LOCALISATION;

	YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_charges_financieres();

	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_charges_financieres,
										 sqlTableModel);

	tableView_lister_charges_financieres
		->SAVE_YERITH_TABLEVIEW_header_COLUMN_order(*this);
}


void YerithAdminListerWindow::RESET_VIEW_utilisateur()
{
	_NOT_VISIBLE_FOR_USER_DB_TABLE_COLUMN_NAME.clear();

	if (!_NOT_VISIBLE_FOR_USER_DB_TABLE_COLUMN_NAME.contains(YerithDatabaseTableColumn::MOT_PASSE))
	{
		_NOT_VISIBLE_FOR_USER_DB_TABLE_COLUMN_NAME
		.append(YerithDatabaseTableColumn::MOT_PASSE);
	}

	_visibleDBColumnNameStrList.clear();

	_visibleDBColumnNameStrList
		<< YerithDatabaseTableColumn::ID
		<< YerithDatabaseTableColumn::PRENOM
		<< YerithDatabaseTableColumn::NOM
		<< YerithDatabaseTableColumn::TITRE
		<< YerithDatabaseTableColumn::ROLE
		<< YerithDatabaseTableColumn::LOCALISATION;

	YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_users();

	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_utilisateur,
										 sqlTableModel);

	tableView_lister_utilisateur
		->SAVE_YERITH_TABLEVIEW_header_COLUMN_order(*this);
}


void YerithAdminListerWindow::RESET_VIEW_localisation()
{
	_visibleDBColumnNameStrList.clear();

	_visibleDBColumnNameStrList
		<< YerithDatabaseTableColumn::ID
		<< YerithDatabaseTableColumn::NUMERO_UNIQUE
		<< YerithDatabaseTableColumn::ADRESSE_IP
		<< YerithDatabaseTableColumn::NOM_LOCALISATION
		<< YerithDatabaseTableColumn::QUARTIER
		<< YerithDatabaseTableColumn::VILLE
		<< YerithDatabaseTableColumn::NUMERO_TELEPHONE_1;

	YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_localisations();

	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_localisation,
										 sqlTableModel);

	tableView_lister_localisation
		->SAVE_YERITH_TABLEVIEW_header_COLUMN_order(*this);
}


void YerithAdminListerWindow::RESET_VIEW_departements_de_produits()
{
	_visibleDBColumnNameStrList.clear();

	_visibleDBColumnNameStrList
		<< YerithDatabaseTableColumn::ID
		<< YerithDatabaseTableColumn::NOM_DEPARTEMENT_PRODUIT
		<< YerithDatabaseTableColumn::DESCRIPTION_DEPARTEMENT_PRODUIT
		<< YerithDatabaseTableColumn::YRI_Not_PHYSICAL_ASSET_STOCK_DEPARTMENT;

	YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_departements_produits();

	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_departements_produits,
										 sqlTableModel);

	tableView_lister_departements_produits
		->SAVE_YERITH_TABLEVIEW_header_COLUMN_order(*this);
}


void YerithAdminListerWindow::RESET_VIEW_categorie()
{
    _visibleDBColumnNameStrList.clear();

    _visibleDBColumnNameStrList
		<< YerithDatabaseTableColumn::ID
		<< YerithDatabaseTableColumn::NOM_CATEGORIE
		<< YerithDatabaseTableColumn::NOM_DEPARTEMENT_PRODUIT
		<< YerithDatabaseTableColumn::DESCRIPTION_CATEGORIE;

	YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_categories();

	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_categorie,
										 sqlTableModel);

	tableView_lister_categorie
		->SAVE_YERITH_TABLEVIEW_header_COLUMN_order(*this);
}


void YerithAdminListerWindow::RESET_VIEW_LIGNE_BUDGETAIRE()
{
	_visibleDBColumnNameStrList.clear();

	_visibleDBColumnNameStrList
		<< YerithDatabaseTableColumn::ID
		<< YerithDatabaseTableColumn::INTITULE_DE_LA_ligne_budgetaire
		<< YerithDatabaseTableColumn::montant_de_la_ligne_budgetaire
		<< YerithDatabaseTableColumn::INTITULE_DU_COMPTE_BANCAIRE
		<< YerithDatabaseTableColumn::DESCRIPTION_ligne_budgetaire;

	YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_lignes_budgetaires();

	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_LIGNE_BUDGETAIRE,
										 sqlTableModel);

	tableView_lister_LIGNE_BUDGETAIRE
		->SAVE_YERITH_TABLEVIEW_header_COLUMN_order(*this);
}


void YerithAdminListerWindow::RESET_VIEW_compte_bancaire()
{
	_visibleDBColumnNameStrList.clear();

	_visibleDBColumnNameStrList
		<< YerithDatabaseTableColumn::ID
		<< YerithDatabaseTableColumn::REFERENCE_DU_COMPTE_BANCAIRE
		<< YerithDatabaseTableColumn::INTITULE_DU_COMPTE_BANCAIRE
		<< YerithDatabaseTableColumn::INSTITUT_BANCAIRE
		<< YerithDatabaseTableColumn::DESCRIPTION_DU_COMPTE_BANCAIRE;

	YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_comptes_bancaires();

	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_compte_bancaire,
										 sqlTableModel);

	tableView_lister_compte_bancaire
		->SAVE_YERITH_TABLEVIEW_header_COLUMN_order(*this);
}


void YerithAdminListerWindow::RESET_VIEW_alerte()
{
	_visibleDBColumnNameStrList.clear();

	_visibleDBColumnNameStrList
		<< YerithDatabaseTableColumn::ID
		<< YerithDatabaseTableColumn::STOCKS_ID
		<< YerithDatabaseTableColumn::DESIGNATION_ALERTE
		<< YerithDatabaseTableColumn::DESIGNATION
		<< YerithDatabaseTableColumn::DATE_DEBUT
		<< YerithDatabaseTableColumn::DATE_FIN
		<< YerithDatabaseTableColumn::DESTINATAIRE
		<< YerithDatabaseTableColumn::DATE_CREATION
		<< YerithDatabaseTableColumn::ALERTE_RESOLUE;

	YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_alertes();

	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_alerte,
										 sqlTableModel);

	tableView_lister_alerte
		->SAVE_YERITH_TABLEVIEW_header_COLUMN_order(*this);
}


void YerithAdminListerWindow::RESET_VIEW_remise()
{
	_visibleDBColumnNameStrList.clear();

	_visibleDBColumnNameStrList
		<< YerithDatabaseTableColumn::ID
		<< YerithDatabaseTableColumn::STOCKS_ID
		<< YerithDatabaseTableColumn::DESIGNATION_ALERTE
		<< YerithDatabaseTableColumn::DESIGNATION
		<< YerithDatabaseTableColumn::DATE_DEBUT
		<< YerithDatabaseTableColumn::DATE_FIN
		<< YerithDatabaseTableColumn::DESTINATAIRE
		<< YerithDatabaseTableColumn::DATE_CREATION
		<< YerithDatabaseTableColumn::ALERTE_RESOLUE;

	YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_remises();

	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_remise,
										 sqlTableModel);

	tableView_lister_remise
		->SAVE_YERITH_TABLEVIEW_header_COLUMN_order(*this);
}


void YerithAdminListerWindow::SETUP_PRINT()
{
    switch (tabWidget_lister->currentIndex())
    {
    case SUJET_ACTION_COMPTE_UTILISATEUR:

        actionExporter_au_format_csv->setToolTip(QObject::tr("Exporter au format csv. 'UTILISATEURS'"));

        actionExporter_au_format_csv->setText(QObject::tr("Exporter au format csv. 'UTILISATEURS'"));

        actionAfficherPDF->setToolTip(QObject::tr("Imprimer la liste affichée. 'UTILISATEURS'"));

        actionAfficherPDF->setText(QObject::tr("Imprimer la liste affichée. 'UTILISATEURS'"));

    	tableView_lister_utilisateur->_currentViewWindow = this;

    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_utilisateur);

    	setup_select_configure_dbcolumn(YerithDatabase::USERS);
        break;


    case SUJET_ACTION_LOCALISATION:

        actionExporter_au_format_csv->setToolTip(QObject::tr("Exporter au format csv. 'Localisations'"));

        actionExporter_au_format_csv->setText(QObject::tr("Exporter au format csv. 'Localisations'"));

        actionAfficherPDF->setToolTip(QObject::tr("Imprimer la liste affichée. 'Localisations'"));

        actionAfficherPDF->setText(QObject::tr("Imprimer la liste affichée. 'Localisations'"));

    	tableView_lister_localisation->_currentViewWindow = this;

    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_localisation);

    	setup_select_configure_dbcolumn(YerithDatabase::LOCALISATIONS);
        break;


    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:

        actionExporter_au_format_csv->setToolTip(QObject::tr("Exporter au format csv. 'DÉPARTEMENTS'"));

        actionExporter_au_format_csv->setText(QObject::tr("Exporter au format csv. 'DÉPARTEMENTS'"));

        actionAfficherPDF->setToolTip(QObject::tr("Imprimer la liste affichée. 'DÉPARTEMENTS'"));

        actionAfficherPDF->setText(QObject::tr("Imprimer la liste affichée. 'DÉPARTEMENTS'"));

    	tableView_lister_departements_produits->_currentViewWindow = this;

    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_departements_produits);

    	setup_select_configure_dbcolumn(YerithDatabase::DEPARTEMENTS_PRODUITS);
        break;


    case SUJET_ACTION_CATEGORIE:

        actionExporter_au_format_csv->setToolTip(QObject::tr("Exporter au format csv. 'Catégories'"));

        actionExporter_au_format_csv->setText(QObject::tr("Exporter au format csv. 'Catégories'"));

        actionAfficherPDF->setToolTip(QObject::tr("Imprimer la liste affichée. 'Catégories'"));

        actionAfficherPDF->setText(QObject::tr("Imprimer la liste affichée. 'Catégories'"));

    	tableView_lister_categorie->_currentViewWindow = this;

    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_categorie);

    	setup_select_configure_dbcolumn(YerithDatabase::CATEGORIES);
        break;


    case SUJET_ACTION_ligne_budgetaire:

        actionExporter_au_format_csv->setToolTip(QObject::tr("Exporter au format csv. 'lignes budgétaires'"));

        actionExporter_au_format_csv->setText(QObject::tr("Exporter au format csv. 'lignes budgétaires'"));

        actionAfficherPDF->setToolTip(QObject::tr("Imprimer la liste affichée. 'lignes budgétaires'"));

        actionAfficherPDF->setText(QObject::tr("Imprimer la liste affichée. 'lignes budgétaires'"));

    	tableView_lister_LIGNE_BUDGETAIRE->_currentViewWindow = this;

    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_LIGNE_BUDGETAIRE);

    	setup_select_configure_dbcolumn(YerithDatabase::LIGNES_BUDGETAIRES);
        break;


    case SUJET_ACTION_COMPTE_BANCAIRE:

        actionExporter_au_format_csv->setToolTip(QObject::tr("Exporter au format csv. 'comptes bancaires'"));

        actionExporter_au_format_csv->setText(QObject::tr("Exporter au format csv. 'comptes bancaires'"));

        actionAfficherPDF->setToolTip(QObject::tr("Imprimer la liste affichée. 'comptes bancaires'"));

        actionAfficherPDF->setText(QObject::tr("Imprimer la liste affichée. 'comptes bancaires'"));

    	tableView_lister_compte_bancaire->_currentViewWindow = this;

    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_compte_bancaire);

    	setup_select_configure_dbcolumn(YerithDatabase::COMPTES_BANCAIRES);
        break;


    case SUJET_ACTION_ALERTE:

        actionExporter_au_format_csv->setToolTip(QObject::tr("Exporter au format csv. 'Alertes'"));

        actionExporter_au_format_csv->setText(QObject::tr("Exporter au format csv. 'Alertes'"));

        actionAfficherPDF->setToolTip(QObject::tr("Imprimer la liste affichée. 'Alertes'"));

        actionAfficherPDF->setText(QObject::tr("Imprimer la liste affichée. 'Alertes'"));

    	tableView_lister_alerte->_currentViewWindow = this;

    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_alerte);

    	setup_select_configure_dbcolumn(YerithDatabase::ALERTES);
        break;


    case SUJET_ACTION_REMISE:

        actionExporter_au_format_csv->setToolTip(QObject::tr("Exporter au format csv. 'remises'"));

        actionExporter_au_format_csv->setText(QObject::tr("Exporter au format csv. 'remises'"));

        actionAfficherPDF->setToolTip(QObject::tr("Imprimer la liste affichée. 'remises'"));

        actionAfficherPDF->setText(QObject::tr("Imprimer la liste affichée. 'remises'"));

    	tableView_lister_remise->_currentViewWindow = this;

    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_remise);

    	setup_select_configure_dbcolumn(YerithDatabase::REMISES);
        break;


    case SUJET_ACTION_CHARGE_FINANCIERE:

        actionExporter_au_format_csv->setToolTip(QObject::tr("Exporter au format csv. 'charges financières'"));

        actionExporter_au_format_csv->setText(QObject::tr("Exporter au format csv. 'charges financières'"));

        actionAfficherPDF->setToolTip(QObject::tr("Imprimer la liste affichée. 'charges financières'"));

        actionAfficherPDF->setText(QObject::tr("Imprimer la liste affichée. 'charges financières'"));

    	tableView_lister_charges_financieres->_currentViewWindow = this;

    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_charges_financieres);

    	setup_select_configure_dbcolumn(YerithDatabase::CHARGES_FINANCIERES);
        break;


    default:
        break;
    }
}


void YerithAdminListerWindow::
	LISTER_CHARGES_FINANCIERES(YerithSqlTableModel *aSqlTableModel /* = 0 */)
{
	SETUP_PRINT();

    _windowName = QString("%1 - %2")
                			.arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
                				 QObject::tr("administration ~ lister ~ CHARGES FINANCIÈRES"));

    //In case, for e.g. there is filtering applied to aSqlTableModel
    if (0 != aSqlTableModel &&
        true == YerithUtils::isEqualCaseInsensitive(YerithDatabase::CHARGES_FINANCIERES,
                                                    aSqlTableModel->sqlTableName()))
    {
    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_charges_financieres,
    										 *aSqlTableModel);

    	_curSearchSqlTableModel = aSqlTableModel;
    }
    else
    {
        setUserCurrentlyFiltered(false);

        YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_charges_financieres();

    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_charges_financieres,
    										 sqlTableModel);

    	_curSearchSqlTableModel = &sqlTableModel;
    }


    _LISTER_tab_TO_tabTitle.insert("LISTER_CHARGES_FINANCIERES", _windowName);

    setWindowTitle(_LISTER_tab_TO_tabTitle.value("LISTER_CHARGES_FINANCIERES"));


    _lastItemSelectedForModification = lastSelectedItemForModification();

    set_admin_rechercher_font();

    APPLY_USER_LOCAL_SETTINGS_PARAMETERS();

    tableView_lister_charges_financieres->selectRow(_lastItemSelectedForModification);
}


void YerithAdminListerWindow::lister_utilisateur(YerithSqlTableModel *aSqlTableModel)
{
	SETUP_PRINT();

    _windowName = QString("%1 - %2")
                			.arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
                				 QObject::tr("administration ~ lister ~ utilisateurs"));

    //In case, for e.g. there is filtering applied to aSqlTableModel
    if (0 != aSqlTableModel &&
        true == YerithUtils::isEqualCaseInsensitive(YerithDatabase::USERS,
                                                    aSqlTableModel->sqlTableName()))
    {
    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_utilisateur,
    										 *aSqlTableModel);

    	_curSearchSqlTableModel = aSqlTableModel;
    }
    else
    {
        setUserCurrentlyFiltered(false);

        YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_users();

    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_utilisateur,
    										 sqlTableModel);

    	_curSearchSqlTableModel = &sqlTableModel;
    }


    _LISTER_tab_TO_tabTitle.insert("lister_utilisateur", _windowName);

    setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_utilisateur"));


    _lastItemSelectedForModification = lastSelectedItemForModification();

    set_admin_rechercher_font();

    APPLY_USER_LOCAL_SETTINGS_PARAMETERS();


    {
        int columnIndexROLE =
            _dbtablecolumnNameToDBColumnIndex.value(YerithDatabaseTableColumn::ROLE);

        int columnIndexTITRE =
            _dbtablecolumnNameToDBColumnIndex.value(YerithDatabaseTableColumn::TITRE);


        //QDEBUG_STRING_OUTPUT_2_N("columnIndexROLE", columnIndexROLE);

        //QDEBUG_STRING_OUTPUT_2_N("columnIndexTITRE", columnIndexTITRE);


        setLeftAligned_FOR_YERITH_PDF_LATEX_PRINTING(columnIndexROLE);

        setLeftAligned_FOR_YERITH_PDF_LATEX_PRINTING(columnIndexTITRE);
    }


    tableView_lister_utilisateur->selectRow(_lastItemSelectedForModification);
}


void YerithAdminListerWindow::lister_localisation(YerithSqlTableModel *
                                                  aSqlTableModel)
{
	SETUP_PRINT();

    _windowName = QString("%1 - %2")
                			.arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
                				 QObject::tr("administration ~ lister ~ localisations"));

    if (0 != aSqlTableModel &&
        true == YerithUtils::isEqualCaseInsensitive(YerithDatabase::LOCALISATIONS,
                                                    aSqlTableModel->sqlTableName()))
    {
    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_utilisateur,
    										 *aSqlTableModel);

    	_curSearchSqlTableModel = aSqlTableModel;
    }
    else
    {
        setSiteCurrentlyFiltered(false);

        YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_localisations();

    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_localisation,
    										 sqlTableModel);

    	_curSearchSqlTableModel = &sqlTableModel;
    }


    _LISTER_tab_TO_tabTitle.insert("lister_localisation", _windowName);

    setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_localisation"));


    _lastItemSelectedForModification = lastSelectedItemForModification();

    set_admin_rechercher_font();

    APPLY_USER_LOCAL_SETTINGS_PARAMETERS();

    tableView_lister_localisation->selectRow(_lastItemSelectedForModification);
}


void YerithAdminListerWindow::lister_departements_de_produits(YerithSqlTableModel *aSqlTableModel)
{
    setCenterAligned_FOR_YERITH_PDF_LATEX_PRINTING
        (YerithDatabaseTableColumn::YRI_Not_PHYSICAL_ASSET_STOCK_DEPARTMENT);

	SETUP_PRINT();

    _windowName = QString("%1 - %2")
                			.arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
                				 QObject::tr("administration ~ lister ~ départements"));


    if (0 != aSqlTableModel &&
        true == YerithUtils::isEqualCaseInsensitive(YerithDatabase::DEPARTEMENTS_PRODUITS,
                                                    aSqlTableModel->sqlTableName()))
    {
    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_departements_produits,
    										 *aSqlTableModel);

    	_curSearchSqlTableModel = aSqlTableModel;

#ifdef YERITH_SERVER

    	YerithERPWindows *allWindows = YerithUtils::getAllWindows();

        YERITH_RUNTIME_VERIFIER *dbusServer = allWindows->dbusServer();
        if (dbusServer)
        {
        	dbusServer->YRI_slot_refresh_SELECT_DB_MYSQL__CALLED
							(QString("%1;%2;%3")
								.arg(aSqlTableModel->sqlTableName(),
									 "src/admin/lister/yerith-erp-admin-lister-window.cpp",
									 QString::number(865)));
        }
#endif

        //qDebug() << QString("++ lister_departements_de_produits | setting new _curSearchSqlTableModel| sql table filter: %1")
        //                              .arg(_curSearchSqlTableModel->filter());
    }
    else
    {
        setProductDepartmentCurrentlyFiltered(false);

        YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_departements_produits();

    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_departements_produits,
    										 sqlTableModel);

    	_curSearchSqlTableModel = &sqlTableModel;

#ifdef YERITH_SERVER

    	YerithERPWindows *allWindows = YerithUtils::getAllWindows();

        YERITH_RUNTIME_VERIFIER *dbusServer = allWindows->dbusServer();
        if (dbusServer)
        {
        	dbusServer->YRI_slot_refresh_SELECT_DB_MYSQL__CALLED
							(QString("%1;%2;%3")
								.arg(sqlTableModel.sqlTableName(),
									 "src/admin/lister/yerith-erp-admin-lister-window.cpp",
									 QString::number(890)));
        }
#endif

    }


    _LISTER_tab_TO_tabTitle.insert("lister_departements_de_produits", _windowName);

    setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_departements_de_produits"));

    //qDebug() << QString("++ lister_categorie | sql table filter: %1")
    //                      .arg(_curSearchSqlTableModel->filter());


    _lastItemSelectedForModification = lastSelectedItemForModification();

    set_admin_rechercher_font();

    APPLY_USER_LOCAL_SETTINGS_PARAMETERS();

    tableView_lister_departements_produits->selectRow(_lastItemSelectedForModification);
}


void YerithAdminListerWindow::lister_categorie(YerithSqlTableModel *aSqlTableModel)
{
	SETUP_PRINT();

    _windowName = QString("%1 - %2")
                			.arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
                				 QObject::tr("administration ~ lister ~ catégories"));

    if (0 != aSqlTableModel &&
        true == YerithUtils::isEqualCaseInsensitive(YerithDatabase::CATEGORIES,
                                                    aSqlTableModel->sqlTableName()))
    {
    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_categorie,
    										 *aSqlTableModel);

    	_curSearchSqlTableModel = aSqlTableModel;

//        _curSearchSqlTableModel = aSqlTableModel;
//        //qDebug() << QString("++ lister_categorie | setting new _curSearchSqlTableModel| sql table filter: %1")
//        //                              .arg(_curSearchSqlTableModel->filter());
    }
    else
    {
        setCategoryCurrentlyFiltered(false);

        YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_categories();

    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_categorie,
    										 sqlTableModel);

    	_curSearchSqlTableModel = &sqlTableModel;
    }


    _LISTER_tab_TO_tabTitle.insert("lister_categorie", _windowName);

    setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_categorie"));

    //qDebug() << QString("++ lister_categorie | sql table filter: %1")
    //                      .arg(_curSearchSqlTableModel->filter());


    _lastItemSelectedForModification = lastSelectedItemForModification();

    set_admin_rechercher_font();

    APPLY_USER_LOCAL_SETTINGS_PARAMETERS();

    tableView_lister_categorie->selectRow(_lastItemSelectedForModification);
}


void YerithAdminListerWindow::lister_LIGNE_BUDGETAIRE(YerithSqlTableModel *aSqlTableModel)
{
	SETUP_PRINT();

    _windowName = QString("%1 - %2")
                			.arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
                				 QObject::tr("administration ~ lister ~ LIGNES BUDÉGTAIRES"));


    if (0 != aSqlTableModel &&
        true == YerithUtils::isEqualCaseInsensitive(YerithDatabase::LIGNES_BUDGETAIRES,
                                                    aSqlTableModel->sqlTableName()))
	{
    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_LIGNE_BUDGETAIRE,
    										 *aSqlTableModel);

    	_curSearchSqlTableModel = aSqlTableModel;
		//qDebug() << QString("++ lister_LIGNE_BUDGETAIRE | setting new _curSearchSqlTableModel| sql table filter: %1")
	        		//                              .arg(_curSearchSqlTableModel->filter());
	}
	else
	{
		setLIGNE_BUDGETAIRE_CurrentlyFiltered(false);

        YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_lignes_budgetaires();

    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_LIGNE_BUDGETAIRE,
    										 sqlTableModel);

    	_curSearchSqlTableModel = &sqlTableModel;
	}


	_LISTER_tab_TO_tabTitle.insert("lister_LIGNE_BUDGETAIRE", _windowName);

	setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_LIGNE_BUDGETAIRE"));

	//qDebug() << QString("++ lister_LIGNE_BUDGETAIRE | sql table filter: %1")
	//                      .arg(_curSearchSqlTableModel->filter());


	_lastItemSelectedForModification = lastSelectedItemForModification();

	set_admin_rechercher_font();

	APPLY_USER_LOCAL_SETTINGS_PARAMETERS();

	tableView_lister_LIGNE_BUDGETAIRE->selectRow(_lastItemSelectedForModification);
}


void YerithAdminListerWindow::lister_compte_bancaire(YerithSqlTableModel *aSqlTableModel)
{
	SETUP_PRINT();

    _windowName = QString("%1 - %2")
                			.arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
                				 QObject::tr("administration ~ lister ~ comptes bancaires"));

    if (0 != aSqlTableModel &&
        true == YerithUtils::isEqualCaseInsensitive(YerithDatabase::COMPTES_BANCAIRES,
                                                    aSqlTableModel->sqlTableName()))
    {
    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_compte_bancaire,
    										 *aSqlTableModel);

    	_curSearchSqlTableModel = aSqlTableModel;
    }
    else
    {
        setBankAccountCurrentlyFiltered(false);

        YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_comptes_bancaires();

    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_compte_bancaire,
    										 sqlTableModel);

    	_curSearchSqlTableModel = &sqlTableModel;
    }


    _LISTER_tab_TO_tabTitle.insert("lister_compte_bancaire", _windowName);

    setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_compte_bancaire"));


    _lastItemSelectedForModification = lastSelectedItemForModification();

    set_admin_rechercher_font();

    APPLY_USER_LOCAL_SETTINGS_PARAMETERS();

    tableView_lister_compte_bancaire->selectRow(_lastItemSelectedForModification);
}


void YerithAdminListerWindow::lister_alerte(YerithSqlTableModel *aSqlTableModel)
{
	SETUP_PRINT();

    _windowName = QString("%1 - %2")
                	.arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
                		 QObject::tr("administration ~ lister ~ alertes"));

    if (0 != aSqlTableModel &&
        true == YerithUtils::isEqualCaseInsensitive(YerithDatabase::ALERTES,
                                                    aSqlTableModel->sqlTableName()))
    {
    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_alerte,
    										 *aSqlTableModel);

    	_curSearchSqlTableModel = aSqlTableModel;
    }
    else
    {
        setAlertCurrentlyFiltered(false);

        YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_alertes();

    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_alerte,
    										 sqlTableModel);

    	_curSearchSqlTableModel = &sqlTableModel;
    }


    _LISTER_tab_TO_tabTitle.insert("lister_alerte", _windowName);

    setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_alerte"));


    _lastItemSelectedForModification = lastSelectedItemForModification();

    set_admin_rechercher_font();

    APPLY_USER_LOCAL_SETTINGS_PARAMETERS();

    tableView_lister_alerte->selectRow(_lastItemSelectedForModification);
}


void YerithAdminListerWindow::lister_remise(YerithSqlTableModel *aSqlTableModel)
{
	SETUP_PRINT();

    _windowName = QString("%1 - %2")
                	.arg(GET_YERITH_ERP_WINDOW_TITLE_MACRO,
                		 QObject::tr("administration ~ lister ~ remises"));

    if (0 != aSqlTableModel &&
        true == YerithUtils::isEqualCaseInsensitive(YerithDatabase::REMISES,
                                                    aSqlTableModel->sqlTableName()))
    {
    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_remise,
    										 *aSqlTableModel);

    	_curSearchSqlTableModel = aSqlTableModel;
    }
    else
    {
        setDiscountCurrentlyFiltered(false);

        YerithSqlTableModel &sqlTableModel = _allWindows->getSqlTableModel_remises();

    	LIST_SHOW_TABLE_VIEW_WITH_PAGINATION(*tableView_lister_remise,
    										 sqlTableModel);

    	_curSearchSqlTableModel = &sqlTableModel;
    }


    _LISTER_tab_TO_tabTitle.insert("lister_remise", _windowName);

    setWindowTitle(_LISTER_tab_TO_tabTitle.value("lister_remise"));


    _lastItemSelectedForModification = lastSelectedItemForModification();

    set_admin_rechercher_font();

    APPLY_USER_LOCAL_SETTINGS_PARAMETERS();

    tableView_lister_remise->selectRow(_lastItemSelectedForModification);
}


bool YerithAdminListerWindow::export_csv_file()
{
    bool success = false;

    QList<int> tableColumnsToIgnore;

    fill_table_columns_to_ignore(tableColumnsToIgnore);


    QString an_admin_File_subject;

    QString an_admin_File_subject_title_STRING;

    YerithTableView *an_admin_lister_yerith_table_view = 0;


    switch (tabWidget_lister->currentIndex())
    {
    case SUJET_ACTION_COMPTE_UTILISATEUR:
        an_admin_File_subject = QObject::tr("utilisateurs");
        an_admin_File_subject_title_STRING = an_admin_File_subject;

        an_admin_lister_yerith_table_view = tableView_lister_utilisateur;
        break;

    case SUJET_ACTION_LOCALISATION:
        an_admin_File_subject = QObject::tr("localisations");
        an_admin_File_subject_title_STRING = an_admin_File_subject;

        an_admin_lister_yerith_table_view = tableView_lister_localisation;
        break;

    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:
        an_admin_File_subject = QObject::tr("départements de produits");
        an_admin_File_subject_title_STRING = QObject::tr("departements-de-produits");

        an_admin_lister_yerith_table_view = tableView_lister_departements_produits;
        break;

    case SUJET_ACTION_CATEGORIE:
        an_admin_File_subject = QObject::tr("catégories");
        an_admin_File_subject_title_STRING = an_admin_File_subject;

        an_admin_lister_yerith_table_view = tableView_lister_categorie;
        break;

    case SUJET_ACTION_ligne_budgetaire:
        an_admin_File_subject = QObject::tr("lignes budgétaires");
        an_admin_File_subject_title_STRING = QObject::tr("lignes-budgetaires");

        an_admin_lister_yerith_table_view = tableView_lister_LIGNE_BUDGETAIRE;
        break;

    case SUJET_ACTION_COMPTE_BANCAIRE:
        an_admin_File_subject = QObject::tr("COMPTES BANCAIRES");
        an_admin_File_subject_title_STRING = QObject::tr("COMPTES-BANCAIRES");

        an_admin_lister_yerith_table_view = tableView_lister_compte_bancaire;
        break;

    case SUJET_ACTION_ALERTE:
        an_admin_File_subject = QObject::tr("alertes");
        an_admin_File_subject_title_STRING = an_admin_File_subject;

        an_admin_lister_yerith_table_view = tableView_lister_alerte;
        break;

    case SUJET_ACTION_REMISE:
        an_admin_File_subject = QObject::tr("REMISES");
        an_admin_File_subject_title_STRING = an_admin_File_subject;

        an_admin_lister_yerith_table_view = tableView_lister_remise;
        break;

    case SUJET_ACTION_CHARGE_FINANCIERE:
        an_admin_File_subject = QObject::tr("CHARGES FINANCIÈRES");
        an_admin_File_subject_title_STRING = QObject::tr("CHARGES-FINANCIERES");

        an_admin_lister_yerith_table_view = tableView_lister_charges_financieres;
        break;

    default:
        break;
    }


    if (0 == an_admin_lister_yerith_table_view)
    {
        return false;
    }


    if (YerithMainWindow::LANGUE_ANGLAISE)
    {
        success = YerithUtils::export_csv_file(*this,
                                               *an_admin_lister_yerith_table_view,
                                               tableColumnsToIgnore,
                                               QString("yerith-erp-ADMIN-LISTING-%1-csv-format")
                                                .arg(an_admin_File_subject_title_STRING),
                                               QString("%1 listing")
                                                .arg(an_admin_File_subject));
    }
    else
    {
        success = YerithUtils::export_csv_file(*this,
                                               *an_admin_lister_yerith_table_view,
                                               tableColumnsToIgnore,
                                               QString("yerith-erp-ADMIN-FICHE-%1-format-csv")
                                                .arg(an_admin_File_subject_title_STRING),
                                               QString("fiche des %1")
                                                .arg(an_admin_File_subject));
    }


    return success;
}


void YerithAdminListerWindow::imprimer_pdf_document_WITH_A_YERITH_PROGRESS_BAR()
{
    if (0 != _yerith_PRINT_UTILITIES_TEX_TABLE)
    {
        _yerith_PRINT_UTILITIES_TEX_TABLE->RESET_NOMBRE_DE_LIGNES_TABLEAU_STANDARD();
    }

    YerithProgressBar((YerithWindowsCommons *) this,
                       QString("rgb(%1)").arg(COLOUR_RGB_STRING_YERITH_WHITE_255_255_255))
                     ((YerithWindowsCommons *) this,
                       &YerithWindowsCommons::imprimer_pdf_document);
}


void YerithAdminListerWindow::handleCurrentChanged(int index)
{
	_allWindows->_adminWindow->_curAdminSujetAction =
			(enum AdminSujetAction) tabWidget_lister->currentIndex();


    switch (tabWidget_lister->currentIndex())
    {
    case SUJET_ACTION_COMPTE_UTILISATEUR:
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, false);

    	disconnect(actionReinitialiserChampsDBVisible, 0, 0, 0);

        connect(actionReinitialiserChampsDBVisible,
        		SIGNAL(triggered()),
				this,
                SLOT(RESET_VIEW_utilisateur()));

        if (!isUserCurrentlyFiltered())
        {
        	lister_utilisateur(_curSearchSqlTableModel);
        }

		break;


    case SUJET_ACTION_LOCALISATION:
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, false);

    	disconnect(actionReinitialiserChampsDBVisible, 0, 0, 0);

        connect(actionReinitialiserChampsDBVisible,
        		SIGNAL(triggered()),
				this,
                SLOT(RESET_VIEW_localisation()));

        if (!isSiteCurrentlyFiltered())
        {
        	lister_localisation(_curSearchSqlTableModel);
        }

        break;


    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, false);

    	disconnect(actionReinitialiserChampsDBVisible, 0, 0, 0);

        connect(actionReinitialiserChampsDBVisible,
        		SIGNAL(triggered()),
				this,
                SLOT(RESET_VIEW_departements_de_produits()));

        if (!isProductDepartmentCurrentlyFiltered())
        {
        	lister_departements_de_produits(_curSearchSqlTableModel);
        }

		break;


    case SUJET_ACTION_CATEGORIE:
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, false);

    	disconnect(actionReinitialiserChampsDBVisible, 0, 0, 0);

        connect(actionReinitialiserChampsDBVisible,
        		SIGNAL(triggered()),
				this,
                SLOT(RESET_VIEW_categorie()));

        if (!isCategoryCurrentlyFiltered())
        {
        	lister_categorie(_curSearchSqlTableModel);
        }

        break;


    case SUJET_ACTION_ligne_budgetaire:
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, false);

    	disconnect(actionReinitialiserChampsDBVisible, 0, 0, 0);

        connect(actionReinitialiserChampsDBVisible,
        		SIGNAL(triggered()),
				this,
                SLOT(RESET_VIEW_LIGNE_BUDGETAIRE()));

        YerithQMessageBox::information(this,
                                       QObject::tr("ligne budgetaire"),
                                       QObject::tr("Créer 1 Ligne budgetaire nécessite "
                                        "déja avoir ouvert 1 compte bancaire "
                                        "et l'avoir aussi enregistrer dans ce "
                                        "progiciel de gestion intégré !"));

        if (!is_LIGNE_BUDGETAIRE_CurrentlyFiltered())
        {
        	lister_LIGNE_BUDGETAIRE(_curSearchSqlTableModel);
        }
        break;


    case SUJET_ACTION_COMPTE_BANCAIRE:
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, false);

    	disconnect(actionReinitialiserChampsDBVisible, 0, 0, 0);

        connect(actionReinitialiserChampsDBVisible,
        		SIGNAL(triggered()),
				this,
                SLOT(RESET_VIEW_compte_bancaire()));

        if (!isBankAccountCurrentlyFiltered())
        {
        	lister_compte_bancaire(_curSearchSqlTableModel);
        }

        break;


    case SUJET_ACTION_REMISE:
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, false);

    	disconnect(actionReinitialiserChampsDBVisible, 0, 0, 0);

        connect(actionReinitialiserChampsDBVisible,
        		SIGNAL(triggered()),
				this,
                SLOT(RESET_VIEW_remise()));

        if (!isDiscountCurrentlyFiltered())
        {
        	lister_remise(_curSearchSqlTableModel);
        }

        break;


    case SUJET_ACTION_ALERTE:
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, false);

    	disconnect(actionReinitialiserChampsDBVisible, 0, 0, 0);

        connect(actionReinitialiserChampsDBVisible,
        		SIGNAL(triggered()),
				this,
                SLOT(RESET_VIEW_alerte()));

        if (!isAlertCurrentlyFiltered())
        {
        	lister_alerte(_curSearchSqlTableModel);
        }

        break;


    case SUJET_ACTION_CHARGE_FINANCIERE:
        YERITH_ERP_WRAPPER_QACTION_SET_ENABLED(action_CREER_A_PARTIR_DE_CECI, true);

    	disconnect(actionReinitialiserChampsDBVisible, 0, 0, 0);

        connect(actionReinitialiserChampsDBVisible,
        		SIGNAL(triggered()),
				this,
                SLOT(RESET_VIEW_CHARGES_FINANCIERES()));

        if (!isCharge_financieres_CurrentlyFiltered())
        {
        	LISTER_CHARGES_FINANCIERES(_curSearchSqlTableModel);
        }

        break;

    default:
        break;
    }

    set_admin_rechercher_font();
}


void YerithAdminListerWindow::creer()
{
    rendreInvisible();

    _allWindows->_adminCreateWindow
        ->rendreVisible(_curStocksTableModel,
                        tabWidget_lister->currentIndex());
}


void YerithAdminListerWindow::handleItemModification(const QModelIndex &index)
{
    //_logger->debug("handleItemModification", QString("model index: %1").arg(index.row()));
    _lastItemSelectedForModification = index.row();
}


void YerithAdminListerWindow::modifier()
{
    _logger->log("modifier",
                 QString("current index: %1").arg(tabWidget_lister->
                                                  currentIndex()));

    switch (tabWidget_lister->currentIndex())
    {
    case SUJET_ACTION_COMPTE_UTILISATEUR:
        if (tableView_lister_utilisateur->rowCount() > 0)
        {
            _allWindows->_adminModifierWindow->rendreVisible(SUJET_ACTION_COMPTE_UTILISATEUR);
            rendreInvisible();
        }
        break;

    case SUJET_ACTION_LOCALISATION:
        if (tableView_lister_localisation->rowCount() > 0)
        {
            _allWindows->_adminModifierWindow->rendreVisible(SUJET_ACTION_LOCALISATION);
            rendreInvisible();
        }
        break;

    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:
        if (tableView_lister_departements_produits->rowCount() > 0)
        {
            //qDebug() << "++ YerithAdminListerWindow::modifier | categorie";

            _allWindows->_adminModifierWindow->rendreVisible(SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS);
            rendreInvisible();
        }
        break;

    case SUJET_ACTION_CATEGORIE:
        if (tableView_lister_categorie->rowCount() > 0)
        {
            //qDebug() << "++ YerithAdminListerWindow::modifier | categorie";

            _allWindows->_adminModifierWindow->rendreVisible(SUJET_ACTION_CATEGORIE);
            rendreInvisible();
        }
        break;

    case SUJET_ACTION_ligne_budgetaire:
        if (tableView_lister_LIGNE_BUDGETAIRE->rowCount() > 0)
        {
            //qDebug() << "++ YerithAdminListerWindow::modifier | LIGNE BUDGÉTAIRE";

            _allWindows->_adminModifierWindow->rendreVisible(SUJET_ACTION_ligne_budgetaire);
            rendreInvisible();
        }
        break;

    case SUJET_ACTION_COMPTE_BANCAIRE:
        if (tableView_lister_compte_bancaire->rowCount() > 0)
        {
            _allWindows->_adminModifierWindow->rendreVisible(SUJET_ACTION_COMPTE_BANCAIRE);
            rendreInvisible();
        }
        break;

    case SUJET_ACTION_ALERTE:
        if (tableView_lister_alerte->rowCount() > 0)
        {
            _allWindows->_adminModifierWindow->rendreVisible(SUJET_ACTION_ALERTE);
            rendreInvisible();
        }
        break;

    case SUJET_ACTION_REMISE:
        if (tableView_lister_remise->rowCount() > 0)
        {
            _allWindows->_adminModifierWindow->rendreVisible(SUJET_ACTION_REMISE);
            rendreInvisible();
        }
        break;

    case SUJET_ACTION_CHARGE_FINANCIERE:
        if (tableView_lister_charges_financieres->rowCount() > 0)
        {
            _allWindows->_adminModifierWindow->rendreVisible(SUJET_ACTION_CHARGE_FINANCIERE);
            rendreInvisible();
        }
        break;

    default:
        break;
    }
}


void YerithAdminListerWindow::afficher_au_detail()
{
    switch (tabWidget_lister->currentIndex())
    {
    case SUJET_ACTION_COMPTE_UTILISATEUR:
        afficher_detail_utilisateur();
        break;

    case SUJET_ACTION_LOCALISATION:
        afficher_detail_localisation();
        break;

    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:
        afficher_detail_departements_de_produits();
        break;

    case SUJET_ACTION_CATEGORIE:
        afficher_detail_categorie();
        break;

    case SUJET_ACTION_ligne_budgetaire:
    	afficher_detail_LIGNE_BUDGETAIRE();
        break;

    case SUJET_ACTION_COMPTE_BANCAIRE:
        afficher_detail_compte_bancaire();
        break;

    case SUJET_ACTION_REMISE:
        afficher_detail_remise();
        break;

    case SUJET_ACTION_ALERTE:
        afficher_detail_alerte();
        break;

    case SUJET_ACTION_CHARGE_FINANCIERE:
    	afficher_detail_CHARGE_FINANCIERE();
        break;

    default:
        break;
    }
}


void YerithAdminListerWindow::afficher_detail_CHARGE_FINANCIERE()
{
    _allWindows->_adminDetailWindow
		->rendreVisible_CHARGE_FINANCIERE(lastSelectedItemForModification());

    rendreInvisible();
}


void YerithAdminListerWindow::afficher_detail_utilisateur()
{
    _allWindows->_adminDetailWindow->rendreVisibleCompteUtilisateur
    (lastSelectedItemForModification());
    rendreInvisible();
}


void YerithAdminListerWindow::afficher_detail_localisation()
{
    _allWindows->_adminDetailWindow->rendreVisibleLocalisation
    (lastSelectedItemForModification());
    rendreInvisible();
}


void YerithAdminListerWindow::afficher_detail_departements_de_produits()
{
    //_logger->debug("afficher_detail_categorie",
    //  QString("lastSelectedItemForModification: %1").arg(lastSelectedItemForModification()));
    _allWindows->_adminDetailWindow->rendreVisibleDepartementsDeProduits
    (lastSelectedItemForModification());
    rendreInvisible();
}


void YerithAdminListerWindow::afficher_detail_categorie()
{
    //_logger->debug("afficher_detail_categorie",
    //  QString("lastSelectedItemForModification: %1").arg(lastSelectedItemForModification()));
    _allWindows->_adminDetailWindow->rendreVisibleCategorie
    (lastSelectedItemForModification());
    rendreInvisible();
}


void YerithAdminListerWindow::afficher_detail_LIGNE_BUDGETAIRE()
{
    //_logger->debug("afficher_detail_categorie",
    //  QString("lastSelectedItemForModification: %1").arg(lastSelectedItemForModification()));
    _allWindows->_adminDetailWindow->rendreVisibleLigneBudgetaire(lastSelectedItemForModification());
    rendreInvisible();
}


void YerithAdminListerWindow::afficher_detail_compte_bancaire()
{
    _allWindows->_adminDetailWindow->rendreVisibleCompteBancaire
    (lastSelectedItemForModification());
    rendreInvisible();
}


void YerithAdminListerWindow::afficher_detail_remise()
{
    _allWindows->_adminDetailWindow->
    rendreVisibleRemise(lastSelectedItemForModification());
    rendreInvisible();
}


void YerithAdminListerWindow::afficher_detail_alerte()
{
    _allWindows->_adminDetailWindow->
    rendreVisibleAlerte(lastSelectedItemForModification());
    rendreInvisible();
}


void YerithAdminListerWindow::supprimer()
{
    switch (tabWidget_lister->currentIndex())
    {
    case SUJET_ACTION_COMPTE_UTILISATEUR:
        supprimer_utilisateur();
        break;

    case SUJET_ACTION_LOCALISATION:
        supprimer_localisation();
        break;

    case SUJET_ACTION_CATEGORIE:
        supprimer_categorie();
        break;

    case SUJET_ACTION_ligne_budgetaire:
    	supprimer_LIGNE_BUDGETAIRE();
        break;

    case SUJET_ACTION_COMPTE_BANCAIRE:
        supprimer_compte_bancaire();
        break;

    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:
        supprimer_departement_de_produit();
        break;

    case SUJET_ACTION_REMISE:
        supprimer_remise();
        break;

    case SUJET_ACTION_ALERTE:
        supprimer_alerte();
        break;

    case SUJET_ACTION_CHARGE_FINANCIERE:
        break;

    default:
        break;
    }
}


void YerithAdminListerWindow::supprimer_utilisateur()
{
    _logger->log("supprimer_utilisateur");

    YerithSqlTableModel *usersTableModel = 0;

    if (_curSearchSqlTableModel &&
    	YerithUtils::isEqualCaseInsensitive(YerithDatabase::USERS,
                                            _curSearchSqlTableModel->sqlTableName()))
    {
        usersTableModel = _curSearchSqlTableModel;
    }
    else
    {
        usersTableModel = &_allWindows->getSqlTableModel_users();
    }

    QSqlRecord record =
                    usersTableModel->record(lastSelectedItemForModification());

    if (record.isEmpty() || record.isNull("nom_utilisateur"))
    {
        return;
    }

    QString prenom(GET_SQL_RECORD_DATA(record, YerithDatabaseTableColumn::PRENOM));
    QString nom(GET_SQL_RECORD_DATA(record, YerithDatabaseTableColumn::NOM));
    QString id(GET_SQL_RECORD_DATA(record, "nom_utilisateur"));

    prenom.append(QString(" %1 (%2)").arg(nom, id));

    QString
    msgConfirmation(QObject::tr("Supprimer l'utilisateur '%1' ?'").arg
                    (prenom));

    if (QMessageBox::Ok ==
            YerithQMessageBox::question(this, QObject::tr("supprimer"),
                                        msgConfirmation, QMessageBox::Cancel,
                                        QMessageBox::Ok))
    {
        bool success =
                        usersTableModel->removeRow(lastSelectedItemForModification());

        if (success)
        {
            QString msg(QObject::tr
                        ("L'utilisateur '%1' a été supprimée de la base de données !").
                        arg(prenom));

            YerithQMessageBox::information(this, QObject::tr("supprimer"),
                                           msg, QMessageBox::Ok);

            self_reset_view(SUJET_ACTION_COMPTE_UTILISATEUR);
        }
        else
        {
            QString msg(QObject::tr
                        ("L'utilisateur '%1 %2' n'a pas été supprimée de la base de données !").
                        arg(prenom, nom));

            YerithQMessageBox::warning(this, QObject::tr("supprimer"),
                                       msg, QMessageBox::Ok);
        }
    }
}


void YerithAdminListerWindow::supprimer_localisation()
{
    _logger->log("supprimer_localisation");

    YerithSqlTableModel *localisationsTableModel = 0;

    if (_curSearchSqlTableModel
            && YerithUtils::isEqualCaseInsensitive(YerithDatabase::LOCALISATIONS,
                                                   _curSearchSqlTableModel->
                                                   sqlTableName()))
    {
        localisationsTableModel = _curSearchSqlTableModel;
    }
    else
    {
        localisationsTableModel =
                        &_allWindows->getSqlTableModel_localisations();
    }

    QSqlRecord record =
                    localisationsTableModel->record(lastSelectedItemForModification());

    if (record.isEmpty() || record.isNull(YerithDatabaseTableColumn::NOM_LOCALISATION))
    {
        return;
    }

    QString nom_localisation(GET_SQL_RECORD_DATA(record, YerithDatabaseTableColumn::NOM_LOCALISATION));

    QString msgConfirmation(QObject::tr("Supprimer la localisation '%1' ?").
                            arg(nom_localisation));

    if (QMessageBox::Ok ==
            YerithQMessageBox::question(this, QObject::tr("supprimer"),
                                        msgConfirmation, QMessageBox::Cancel,
                                        QMessageBox::Ok))
    {
        bool success =
                        localisationsTableModel->removeRow(lastSelectedItemForModification
                                                           ());

        QString msg(QObject::tr("La localisation '%1").
                    arg(nom_localisation));

        if (success)
        {
            msg.append(QObject::tr
                       ("' a été supprimée de la base de données !"));

            YerithQMessageBox::information(this, QObject::tr("supprimer"),
                                           msg, QMessageBox::Ok);

            self_reset_view(SUJET_ACTION_LOCALISATION);
        }
        else
        {
            msg.append(QObject::tr
                       (" n'a pas été supprimée de la base de données !"));

            YerithQMessageBox::warning(this, QObject::tr("supprimer"),
                                       msg, QMessageBox::Ok);
        }
    }
}


void YerithAdminListerWindow::supprimer_categorie()
{
    _logger->log("supprimer_categorie");

    YerithSqlTableModel *categoriesTableModel = 0;

    if (_curSearchSqlTableModel
            && YerithUtils::isEqualCaseInsensitive(YerithDatabase::CATEGORIES,
                                                   _curSearchSqlTableModel->
                                                   sqlTableName()))
    {
        categoriesTableModel = _curSearchSqlTableModel;
    }
    else
    {
        categoriesTableModel = &_allWindows->getSqlTableModel_categories();
    }

    QSqlRecord record =
                    categoriesTableModel->record(lastSelectedItemForModification());

    if (record.isEmpty()
            || record.isNull(YerithDatabaseTableColumn::NOM_CATEGORIE))
    {
        return;
    }

    QString
    categorie(GET_SQL_RECORD_DATA
              (record, YerithDatabaseTableColumn::NOM_CATEGORIE));

    QString msgConfirmation(QObject::tr("Supprimer la catégorie '%1' ?").
                            arg(categorie));

    if (QMessageBox::Ok ==
            YerithQMessageBox::question(this, QObject::tr("supprimer"),
                                        msgConfirmation, QMessageBox::Cancel,
                                        QMessageBox::Ok))
    {
    	/*
    	 * CHECK UNDER STOCK TABLE
    	 */
        QString
        SEARCH_IF_STOCK_UNDER_TO_REMOVE_CATEGORIE_STILL_EXIST(QString
                                                              ("select %1 from %2 where %3='%4'")
															  .arg(YerithDatabaseTableColumn::ID,
																   YerithDatabase::STOCKS,
																   YerithDatabaseTableColumn::CATEGORIE,
																   categorie));


        int existing_stock_row_count =
                        YerithUtils::execQueryRowCount
                        (SEARCH_IF_STOCK_UNDER_TO_REMOVE_CATEGORIE_STILL_EXIST);


        if (existing_stock_row_count > 0)
        {
            YerithQMessageBox::information(this,
                                           QObject::tr("supprimer"),
                                           QObject::tr
                                           ("1. La catégorie '%1' comporte"
                                            " encore '%2' stocks !\n\n"
                                            "2. SVP, supprimer ou ecouler d'abord"
                                            " tous ces stocks !").arg
                                           (categorie,
                                            QString::number
                                            (existing_stock_row_count)),
                                           QMessageBox::Ok);

            //#define YERITH_DEVEL_TARGET
            #ifndef YERITH_DEVEL_TARGET
            return;
            #endif // YERITH_DEVEL_TARGET
        }

    	/*
    	 * CHECK UNDER "charges_financieres" TABLE
    	 */
        QString
        SEARCH_IF_STOCK_UNDER_TO_REMOVE_charges_financieres_STILL_EXIST(QString
                                                              ("select %1 from %2 where %3='%4'")
															  .arg(YerithDatabaseTableColumn::ID,
																   YerithDatabase::CHARGES_FINANCIERES,
																   YerithDatabaseTableColumn::CATEGORIE,
																   categorie));


        int existing_charges_financieres_row_count =
                        YerithUtils::execQueryRowCount
                        (SEARCH_IF_STOCK_UNDER_TO_REMOVE_charges_financieres_STILL_EXIST);


        if (existing_charges_financieres_row_count > 0)
        {
            YerithQMessageBox::information(this,
                                           QObject::tr("supprimer"),
                                           QObject::tr
                                           ("1. La catégorie '%1' comporte"
                                            " encore '%2' charges financières !\n\n"
                                            "2. SVP, supprimer d'abord tous ces charges financières !").arg
                                           (categorie,
                                            QString::number(existing_charges_financieres_row_count)),
                                            QMessageBox::Ok);

            //#define YERITH_DEVEL_TARGET
            #ifndef YERITH_DEVEL_TARGET
            return;
            #endif // YERITH_DEVEL_TARGET
        }



        bool success =
                        categoriesTableModel->removeRow(lastSelectedItemForModification());

        QString msg(QObject::tr("La catégorie '%1").arg(categorie));

        if (success)
        {
#ifdef YERITH_SERVER

        YERITH_RUNTIME_VERIFIER *dbusServer = _allWindows->dbusServer();
        if (dbusServer)
        {
        	dbusServer->YRI_slot_refresh_DELETE_DB_MYSQL__CALLED
							(QString("%1.%2;%3;%4")
								.arg(YerithDatabase::CATEGORIES,
									 categorie,
									 "src/admin/lister/yerith-erp-admin-lister-window.cpp",
									 QString::number(2124)));
        }
#endif //YERITH_SERVER

            msg.append(QObject::tr
                       ("' a été supprimé de la base de données !"));

            YerithQMessageBox::information(this, QObject::tr("supprimer"),
                                           msg, QMessageBox::Ok);

            self_reset_view(SUJET_ACTION_CATEGORIE);
        }
        else
        {
            msg.append(QObject::tr
                       (" n'a pas pu être supprimé de la base de données !"));

            YerithQMessageBox::warning(this, tr("supprimer"), msg,
                                       QMessageBox::Ok);
        }
    }
}


void YerithAdminListerWindow::supprimer_LIGNE_BUDGETAIRE()
{
	_logger->log("supprimer_LIGNE_BUDGETAIRE");

	YerithSqlTableModel *lignesBudgetairesTableModel = 0;

	if (_curSearchSqlTableModel &&
		YerithUtils::isEqualCaseInsensitive(YerithDatabase::LIGNES_BUDGETAIRES,
											_curSearchSqlTableModel->sqlTableName()))
	{
		lignesBudgetairesTableModel = _curSearchSqlTableModel;
	}
	else
	{
		lignesBudgetairesTableModel = &_allWindows->getSqlTableModel_lignes_budgetaires();
	}

	QSqlRecord record = lignesBudgetairesTableModel->record(lastSelectedItemForModification());

	if (record.isEmpty() ||
		record.isNull(YerithDatabaseTableColumn::INTITULE_DE_LA_ligne_budgetaire))
	{
		return;
	}

	QString intitule_de_la_ligne_budgetaire(
				GET_SQL_RECORD_DATA(record,
								    YerithDatabaseTableColumn::INTITULE_DE_LA_ligne_budgetaire));

	QString msgConfirmation(QObject::tr("Supprimer la LIGNE BUDGÉTAIRE '%1' ?")
								.arg(intitule_de_la_ligne_budgetaire));

	if (QMessageBox::Ok ==
			YerithQMessageBox::question(this, QObject::tr("supprimer"),
					msgConfirmation, QMessageBox::Cancel,
					QMessageBox::Ok))
	{
		/*
		 * CHECK UNDER CHARGES_FINANCIERES TABLE
		 *
		 * column 'categorie' of table charges_financieres corresponds
		 * to column 'INTITULE_DE_LA_ligne_budgetaire' of table LIGNES_BUDGETAIRES !
		 */
		QString SEARCH_IF_financial_expense_UNDER_TO_REMOVE_BUDGET_LINE_STILL_EXIST(
				QString("select %1 from %2 where %3='%4'")
					.arg(YerithDatabaseTableColumn::ID,
						 YerithDatabase::CHARGES_FINANCIERES,
						 YerithDatabaseTableColumn::CATEGORIE,
						 intitule_de_la_ligne_budgetaire));

		int existing_financial_expense_row_count =
				YerithUtils::execQueryRowCount
				(SEARCH_IF_financial_expense_UNDER_TO_REMOVE_BUDGET_LINE_STILL_EXIST);


		if (existing_financial_expense_row_count > 0)
		{
			YerithQMessageBox::information(this,
										   QObject::tr("supprimer"),
										   QObject::tr("1. La LIGNE BUDGÉTAIRE '%1' comporte"
												   	   	   " encore '%2' charges financières !\n\n"
												   	   	   "2. SVP, supprimer d'abord toutes ces "
												   	   	   " charges financières !")
												.arg(intitule_de_la_ligne_budgetaire,
													 QString::number(existing_financial_expense_row_count)),
													 QMessageBox::Ok);

			return;
		}


		bool success = lignesBudgetairesTableModel->removeRow(lastSelectedItemForModification());


		/*
		 * IL FAUT AUSSI SUPPRIMER LA CATÉGORIE CORRESPONDANT !
		 */
		QString DELETE_CORRESPONDING_STOCK_CATEGORY =
				QString("delete from %1 where %2='%3'")
					.arg(YerithDatabase::CATEGORIES,
						 YerithDatabaseTableColumn::NOM_CATEGORIE,
						 intitule_de_la_ligne_budgetaire);

		YerithUtils::execQuery(DELETE_CORRESPONDING_STOCK_CATEGORY);


		QString msg(QObject::tr("La LIGNE BUDGÉTAIRE '%1")
							.arg(intitule_de_la_ligne_budgetaire));

		if (success)
		{
			msg.append(QObject::tr
					("' a été supprimé de la base de données !"));

			YerithQMessageBox::information(this, QObject::tr("supprimer"),
					msg, QMessageBox::Ok);

			self_reset_view(SUJET_ACTION_ligne_budgetaire);
		}
		else
		{
			msg.append(QObject::tr
					(" n'a pas pu être supprimé de la base de données !"));

			YerithQMessageBox::warning(this, tr("supprimer"), msg,
					QMessageBox::Ok);
		}
	}
}


void YerithAdminListerWindow::supprimer_compte_bancaire()
{
    _logger->log("supprimer_compte_bancaire");

    YerithSqlTableModel *comptesBancairesTableModel = 0;

    if (_curSearchSqlTableModel
            && YerithUtils::isEqualCaseInsensitive(YerithDatabase::
                                                   COMPTES_BANCAIRES,
                                                   _curSearchSqlTableModel->
                                                   sqlTableName()))
    {
        comptesBancairesTableModel = _curSearchSqlTableModel;
    }
    else
    {
        comptesBancairesTableModel =
                        &_allWindows->getSqlTableModel_comptes_bancaires();
    }

    QSqlRecord record =
                    comptesBancairesTableModel->record(lastSelectedItemForModification());

    if (record.isEmpty()
            || record.
            isNull(YerithDatabaseTableColumn::REFERENCE_DU_COMPTE_BANCAIRE))
    {
        return;
    }

    QString
    reference_du_compte_bancaire(GET_SQL_RECORD_DATA
                                 (record,
                                  YerithDatabaseTableColumn::
                                  REFERENCE_DU_COMPTE_BANCAIRE));

    QString msgConfirmation(QObject::tr
                            ("Supprimer le compte bancaire avec la référence '%1' ?").
                            arg(reference_du_compte_bancaire));

    if (QMessageBox::Ok ==
            YerithQMessageBox::question(this, QObject::tr("supprimer"),
                                        msgConfirmation, QMessageBox::Cancel,
                                        QMessageBox::Ok))
    {
        bool success =
                        comptesBancairesTableModel->removeRow(lastSelectedItemForModification
                                                              ());

        QString
        msg(QObject::tr
            ("Le compte bancaire avec la référence '%1").arg
            (reference_du_compte_bancaire));

        if (success)
        {
            msg.append(QObject::tr
                       ("' a été supprimée de la base de données !"));

            YerithQMessageBox::information(this, QObject::tr("supprimer"),
                                           msg, QMessageBox::Ok);

            self_reset_view(SUJET_ACTION_COMPTE_BANCAIRE);
        }
        else
        {
            msg.append(QObject::tr
                       (" n'a pas été supprimée de la base de données !"));

            YerithQMessageBox::warning(this, QObject::tr("supprimer"), msg,
                                       QMessageBox::Ok);
        }
    }
}


void YerithAdminListerWindow::supprimer_departement_de_produit()
{
    _logger->log("supprimer_departement_de_produit");

    YerithSqlTableModel *departements_produits_TableModel = 0;

    if (0 != _curSearchSqlTableModel &&
            YerithUtils::isEqualCaseInsensitive(YerithDatabase::
                                                DEPARTEMENTS_PRODUITS,
                                                _curSearchSqlTableModel->
                                                sqlTableName()))
    {
        departements_produits_TableModel = _curSearchSqlTableModel;
    }
    else
    {
        departements_produits_TableModel =
                        &_allWindows->getSqlTableModel_departements_produits();
    }


    QSqlRecord record =
                    departements_produits_TableModel->record(lastSelectedItemForModification
                                                             ());


    if (record.isEmpty()
            || record.isNull(YerithDatabaseTableColumn::NOM_DEPARTEMENT_PRODUIT))
    {
        return;
    }

    QString
    nom_departement_produit(GET_SQL_RECORD_DATA
                            (record,
                             YerithDatabaseTableColumn::
                             NOM_DEPARTEMENT_PRODUIT));

    QString msgConfirmation(QObject::tr
                            ("Supprimer le département de produits '%1' ?").
                            arg(nom_departement_produit));

    if (QMessageBox::Ok ==
            YerithQMessageBox::question(this, QObject::tr("supprimer"),
                                        msgConfirmation, QMessageBox::Cancel,
                                        QMessageBox::Ok))
    {

//#define YERITH_DEVEL_TARGET
#ifndef YERITH_DEVEL_TARGET //FOR SOFTWARE TESTING PURPOSES
    	QString
        SEARCH_IF_STOCK_UNDER_TO_REMOVE_DEPARTMENT_STILL_EXIST
				(QString("select %1 from %2 where %3='%4'")
					.arg(YerithDatabaseTableColumn::ID,
                        YerithDatabase::STOCKS,
						 YerithDatabaseTableColumn::NOM_DEPARTEMENT_PRODUIT,
						 nom_departement_produit));


        int existing_stock_row_count =
                        YerithUtils::execQueryRowCount
                        (SEARCH_IF_STOCK_UNDER_TO_REMOVE_DEPARTMENT_STILL_EXIST);


        if (existing_stock_row_count > 0)
        {
#ifdef YERITH_SERVER
        YERITH_RUNTIME_VERIFIER *dbusServer = _allWindows->dbusServer();
        if (dbusServer)
        {
        	dbusServer->YRI_slot_refresh_SELECT_DB_MYSQL__CALLED
							(QString("%1.%2;%3;%4")
								.arg(YerithDatabase::STOCKS,
									 nom_departement_produit,
									 "src/admin/lister/yerith-erp-admin-lister-window.cpp",
									 QString::number(2406)));
        }
#endif
            YerithQMessageBox::information(this,
                                           QObject::tr("supprimer"),
                                           QObject::tr
                                           ("1. Le département de produits '%1' comporte"
                                            " encore '%2' stocks !\n\n"
                                            "2. SVP, supprimer ou ecouler d'abord"
                                            " tous ces  stocks !").arg
                                           (nom_departement_produit,
                                            QString::number
                                            (existing_stock_row_count)),
                                           QMessageBox::Ok);

            return;
        }
#endif //YERITH_DEVEL_TARGET

        bool successRemoveDEPARTEMENT = departements_produits_TableModel
        									->removeRow(lastSelectedItemForModification());

#ifdef YERITH_SERVER
        YERITH_RUNTIME_VERIFIER *dbusServer = _allWindows->dbusServer();
        if (dbusServer && successRemoveDEPARTEMENT)
        {
        	dbusServer->YRI_slot_refresh_DELETE_DB_MYSQL__CALLED
							(QString("%1.%2;%3;%4")
								.arg(YerithDatabase::DEPARTEMENTS_PRODUITS,
									 nom_departement_produit,
									 "src/admin/lister/yerith-erp-admin-lister-window.cpp",
									 QString::number(2441)));
        }
#endif

        QString STR_QUERY_REMOVE_DEPARTEMENT_DE_PRODUITS_DU_TABLEAU_MARCHANDISES
        (QString("DELETE FROM %1 WHERE %2='%3'").arg
         (YerithDatabase::MARCHANDISES,
          YerithDatabaseTableColumn::NOM_DEPARTEMENT_PRODUIT,
          nom_departement_produit));


        bool successRemoveDEPARTEMENT_marchandises =
        		YerithUtils::execQuery(STR_QUERY_REMOVE_DEPARTEMENT_DE_PRODUITS_DU_TABLEAU_MARCHANDISES);

        successRemoveDEPARTEMENT = successRemoveDEPARTEMENT &&
        		successRemoveDEPARTEMENT_marchandises;

        QString msg(QObject::tr("Le département de produits '%1").arg
                    (nom_departement_produit));

        if (successRemoveDEPARTEMENT_marchandises)
        {
#ifdef YERITH_SERVER
        YERITH_RUNTIME_VERIFIER *dbusServer = _allWindows->dbusServer();
        if (dbusServer && successRemoveDEPARTEMENT_marchandises)
        {
        	dbusServer->YRI_slot_refresh_DELETE_DB_MYSQL__CALLED
							(QString("%1.%2;%3;%4")
								.arg(YerithDatabase::MARCHANDISES,
									 nom_departement_produit,
									 "src/admin/lister/yerith-erp-admin-lister-window.cpp",
									 QString::number(2464)));
        }
#endif
            msg.append(QObject::tr
                       ("' a été supprimé de la base de données !"));

            YerithQMessageBox::information(this, QObject::tr("supprimer"),
                                           msg, QMessageBox::Ok);

            self_reset_view(SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS);
        }
        else
        {
            msg.append(QObject::tr
                       (" n'a pas pu être supprimé de la base de données !"));

            YerithQMessageBox::warning(this, tr("supprimer"), msg,
                                       QMessageBox::Ok);
        }
    }
}


void YerithAdminListerWindow::supprimer_remise()
{

}


void YerithAdminListerWindow::supprimer_alerte()
{
    _logger->log("supprimer_alerte");
    YerithSqlTableModel *alertesTableModel = 0;

    if (_curSearchSqlTableModel
            && YerithUtils::isEqualCaseInsensitive(YerithDatabase::ALERTES,
                                                   _curSearchSqlTableModel->
                                                   sqlTableName()))
    {
        alertesTableModel = _curSearchSqlTableModel;
    }
    else
    {
        alertesTableModel = &_allWindows->getSqlTableModel_alertes();
    }

    QSqlRecord record =
                    alertesTableModel->record(lastSelectedItemForModification());

    if (record.isEmpty()
            || record.isNull(YerithDatabaseTableColumn::DESIGNATION_ALERTE))
    {
        return;
    }

    QString
    designation_alerte(GET_SQL_RECORD_DATA
                       (record,
                        YerithDatabaseTableColumn::DESIGNATION_ALERTE));

    QString msgConfirmation(QObject::tr("Supprimer l'alerte '%1' ?").arg
                            (designation_alerte));

    if (QMessageBox::Ok ==
            YerithQMessageBox::question(this,
                                        QObject::tr("supprimer"),
                                        msgConfirmation,
                                        QMessageBox::Cancel, QMessageBox::Ok))
    {
        bool success =
                        alertesTableModel->removeRow(lastSelectedItemForModification());

        QString msg("L'alerte '");
        msg.append(designation_alerte);

        if (success)
        {
            msg.append(QObject::tr
                       ("' a été supprimée de la base de données !"));

            YerithQMessageBox::information(this,
                                           QObject::tr("supprimer"),
                                           msg, QMessageBox::Ok);

            self_reset_view(SUJET_ACTION_ALERTE);
        }
        else
        {
            msg.append(QObject::tr
                       (" n'a pas été supprimée de la base de données !"));

            YerithQMessageBox::warning(this,
                                       QObject::tr("supprimer"),
                                       msg, QMessageBox::Ok);
        }
    }
}


QString YerithAdminListerWindow::get_latex_template_print_pdf_content()
{
    _documentSpecificElements_FOR_PDF_LATEX_PRINTING.clear();

    switch (tabWidget_lister->currentIndex())
    {
    case SUJET_ACTION_COMPTE_UTILISATEUR:
    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_utilisateur);


    	if (YerithMainWindow::LANGUE_ANGLAISE)
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"User");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"USER LISTING");
        }
        else
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Utilisateurs");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"1 LISTE D'UTILISATEURS");
        }

		break;

    case SUJET_ACTION_LOCALISATION:
    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_localisation);

    	if (YerithMainWindow::LANGUE_ANGLAISE)
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Site");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"SITE LISTING");
        }
        else
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Localisations");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"1 LISTE DE LOCALISATIONS");
        }

        break;

    case SUJET_ACTION_DEPARTEMENTS_DE_PRODUITS:
    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_departements_produits);

    	if (YerithMainWindow::LANGUE_ANGLAISE)
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Department");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"DEPARTMENT LISTING");
        }
        else
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Départements");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"1 LISTE DE DÉPARTEMENTS");
        }

		break;

    case SUJET_ACTION_CATEGORIE:
    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_categorie);

    	if (YerithMainWindow::LANGUE_ANGLAISE)
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Category");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"CATEGORY LISTING");
        }
        else
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Catégories");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"1 LISTE DE CATÉGORIES");
        }

        break;

    case SUJET_ACTION_ligne_budgetaire:
    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_LIGNE_BUDGETAIRE);

    	if (YerithMainWindow::LANGUE_ANGLAISE)
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"BUDGET LINE");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"BUDGET LINE LISTING");
        }
        else
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"LIGNES BUDÉTAIRES");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"1 LISTE DE LIGNES BUDÉTAIRES");
        }

        break;

    case SUJET_ACTION_COMPTE_BANCAIRE:
    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_compte_bancaire);

    	if (YerithMainWindow::LANGUE_ANGLAISE)
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Bank account");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"BANK ACCOUNT LISTING");
        }
        else
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Comptes bancaires");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"1 LISTE DE COMPTES BANCAIRES");
        }

        break;

    case SUJET_ACTION_REMISE:
    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_remise);

    	if (YerithMainWindow::LANGUE_ANGLAISE)
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Discount");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"DISCOUNT LISTING");
        }
        else
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Remises");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"1 LISTE DE REMISES");
        }

        break;

    case SUJET_ACTION_ALERTE:
    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_alerte);

    	if (YerithMainWindow::LANGUE_ANGLAISE)
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Alert");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"ALERT LISTING");
        }
        else
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Alertes");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"1 LISTE D'ALERTES");
        }

        break;

    case SUJET_ACTION_CHARGE_FINANCIERE:
    	setYerithTableView_FROM_WINDOWS_COMMONS(tableView_lister_charges_financieres);

        if (YerithMainWindow::LANGUE_ANGLAISE)
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Financial expense");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"FINANCIAL EXPENSE LISTING");
        }
        else
        {
    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHOBJECT",
						"Charges financières");

    		_documentSpecificElements_FOR_PDF_LATEX_PRINTING
				.insert("YERITHSUBJECT",
						"1 LISTE DE CHARGES FINANCIÈRES");
        }
        break;

    default:
        break;
    }


    if (YerithMainWindow::LANGUE_ANGLAISE)
    {
    	_latex_template_print_pdf_content = YerithUtils::EN_template_lister_admin_objects_TEX;
    }
    else
    {
    	_latex_template_print_pdf_content = YerithUtils::FR_template_lister_admin_objects_TEX;
    }


    return _latex_template_print_pdf_content;
}


void YerithAdminListerWindow::set_shortcuts()
{
    setupShortcut__INCREASE_FONT_SIZE(*actionAUGMENTER_LA_POLICE_DU_TABLEAU);

    setupShortcut__DECREASE_FONT_SIZE(*actiondiminuer_la_police_du_tableau);


    setupShortcutActionExporterAuFormatCsv(*actionExporter_au_format_csv);


    action_rechercher_tableau_liste->setShortcut(YerithUtils::RECHERCHER_QKEYSEQUENCE);
}


void YerithAdminListerWindow::contextMenuEvent(QContextMenuEvent *event)
{
	if (((int) SUJET_ACTION_CHARGE_FINANCIERE) == tabWidget_lister->currentIndex())
	{
		if (tableView_lister_charges_financieres->rowCount() > 0)
		{
			QMenu menu(this);
			menu.setPalette(toolBar_adminListerWindow->palette());
			menu.addAction(action_CREER_A_PARTIR_DE_CECI);
			menu.exec(event->globalPos());
		}
	}
}


