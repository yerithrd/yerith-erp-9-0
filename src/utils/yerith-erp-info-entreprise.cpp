/*
 * yerith-erp-info-entreprise.cpp
 *
 *      Author: Pr. Prof. Dr.-Ing. Xavier Noundou
 */

#include "yerith-erp-info-entreprise.hpp"
...
{
    QString
    yerithEntrepriseLogoImageFile_FULL_PATH(QString("%1/%2").arg
                                            (YerithERPConfig::
                                             temporaryFilesDir,
                                             "YERITH-ENTREPRISE-LOGO.JPG"));

    YerithERPConfig::pathToPdfCOMPANY_LOGO =
                    yerithEntrepriseLogoImageFile_FULL_PATH;

    YerithUtils::CREATE_AN_EMPTY_FILE(YerithERPConfig::pathToPdfCOMPANY_LOGO);


    YerithERPWindows *
    allWindows = YerithUtils::getAllWindows();
    ...
    YerithUtils::loadPixmapFromDB(*a_temporary_label_for_company_logo, img,
                                  "JPG");

    if (0 != a_temporary_label_for_company_logo->pixmap())
    {
        YerithUtils::savePixmapToFile(yerithEntrepriseLogoImageFile_FULL_PATH,
                                      *
    ...
    {
        YERITH_DELETE_FREE_POINTER_NOW(a_temporary_label_for_company_logo)
    }
...
void YerithInfoEntreprise::updateInfoEntrepriseFromDB()
{
...
QString YerithInfoEntreprise::toString()
{
...
QString YerithInfoEntreprise::getNomCommercial_truncated_FOR_SMALL_RECEIPT()
{
...
QString YerithInfoEntreprise::getLocalisation_truncated_FOR_SMALL_RECEIPT_LATEX()
const
{
    QString
    localisation_truncated(YerithUtils::LATEX_IN_OUT_handleForeignAccents
                           (_localisation));
